...
def translationException (access : (MemoryAccessType mem_payload)) (err : PTW_Error) : SailM ExceptionType := do
  ...
  | (.Load VectorPayload, .PTW_No_Access ()) => (pure (E_Load_Access_Fault ()))
  | (.Load VectorPayload, _) => (pure (E_Load_Page_Fault ()))
  ...
  | (.Store VectorPayload, .PTW_No_Access ()) => (pure (E_SAMO_Access_Fault ()))
  | (.Store VectorPayload, _) => (pure (E_SAMO_Page_Fault ()))
