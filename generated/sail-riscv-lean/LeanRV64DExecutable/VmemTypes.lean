...
def undefined_mem_payload (_ : Unit) : SailM mem_payload := do
  (internal_pick [Data, VectorPayload, PageTableEntry, ShadowStack])
...
def is_vector_access (access : (MemoryAccessType mem_payload)) : Bool :=
  match access with
  | .Load VectorPayload => true
  | .Store VectorPayload => true
  | _ => false
