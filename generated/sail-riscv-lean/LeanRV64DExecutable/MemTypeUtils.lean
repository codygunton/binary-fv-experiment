...
def accessFaultFromAccessType (access : (MemoryAccessType mem_payload)) : SailM ExceptionType := do
  ...
  | .Load VectorPayload => (pure (E_Load_Access_Fault ()))
  ...
  | .Store VectorPayload => (pure (E_SAMO_Access_Fault ()))
