#!/usr/bin/env python3
"""Row C: SOUND static unreachability analysis for the occurrences no input exercises.
...
The goal is a statement about three occurrences of the UNCHANGED production ELF:
...
an indirect transfer through a register. So the claim decomposes into:
  ...
  (C) The conclusion is reported per occurrence, together with the residual hypotheses it rests on.
...
----------------------------------------
  ...
  H2 *Def-chain shape.*  Every indirect site's target register is defined only by loads with constant
     ...
     UNRESOLVED and its occurrence is NOT claimed unreachable).

`allThreeStaticallyUnreachable` is deliberately NOT asserted by this script; it emits the exact
per-occurrence verdict and the hypotheses, and the caller decides. Diagnostic-only: never imported by
the theorem graph.
"""
...
def main() -> int:
    ...
    uncovered = {i: occ[i] for i in range(len(occ))
                 if occ[i]["qualified"].split(".")[-1] in
    ...
    global DANGER_SEED
    DANGER_SEED = {o["entryPc"] for o in uncovered.values()}
    ...
    verdicts = {}
    for i, o in sorted(uncovered.items()):
        name = o["qualified"].split(".")[-1]
        in_image = locations_holding(segs, {o["entryPc"]})
        materialized = o["entryPc"] in consts
        verdicts[i] = {
            "index": i, "routine": name, "entryPc": o["entryPc"],
            "directlyReferenced": [d for d in direct_refs if d["target"] == o["entryPc"]],
...
def emit_md(out, uncovered) -> str:
    s = out["summary"]
    L = ["# Row C — static unreachability of the uncovered occurrences (GENERATED)",
         "",
