#!/usr/bin/env python3
"""Row C: SOUND static unreachability analysis for the function_instances no input exercises.
...
The goal is a statement about three function_instances of the UNCHANGED production ELF:
...
----------------------------------------
  ...
  H2 *Def-chain shape.*  Every indirect site's target register is defined only by loads with constant
     ...
     UNRESOLVED and its function_instance is NOT claimed unreachable).

`allThreeStaticallyUnreachable` is deliberately NOT asserted by this script; it emits the exact
per-function_instance verdict and the hypotheses, and the caller decides. Diagnostic-only: never imported by
the theorem graph.
"""
...
def emit_md(out, uncovered) -> str:
    s = out["summary"]
    L = ["# Row C — static unreachability of the uncovered function_instances (GENERATED)",
         "",
    ...
    L += ["",
          ...
          "Row C's conclusions exclude these function_instances either way: they carry no per-function_instance",
