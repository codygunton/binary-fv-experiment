...
def main() -> int:
    ...
    for r in allocating:
        if not so.ledger_agrees(r["facts"]["ledgerObserved"], r["facts"]["ledgerExpected"]):
            failures.append(f"function instance {r['index']}: the real function_instance ledger does not agree")
    ...
    print(f"scaled negative tests OK: {len(derived_rows)} loop-derived rows and "
          f"{len(ledgers)} arm ledgers + {len(allocating)} function_instance ledgers hold on the real "
