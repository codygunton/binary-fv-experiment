#!/usr/bin/env python3
"""Typed per-source_function validation vectors for the Row B direct-source contract check.
...
error (`invalidSsz` / `unknownFork` / `outOfMemory`). The same vectors drive:
  ...
  * the host probe (`ssz_contract_probe.zig --source-function-vectors`), which calls the real private source_function
    through the validation overlay and must match the expected value/error exactly; and
  * a Lean check (`SourceFunctionMeaningVectors.lean`), which evaluates the handwritten `meaning*` and must
    match the same expected value/error by `native_decide`.

So expected ≡ Zig-source_function ∧ expected ≡ Lean-meaning ⇒ Zig-source_function ≡ handwritten meaning, per source_function,
...
JSONL row schema (`ssz-source-function-vectors-v1`), keys sorted, one line per case:
  {"schema","id","source_function","args":{...typed...},
...
SCHEMA = "ssz-source-function-vectors-v1"
...
# --- deterministic SSZ reference (matches both the Zig source_function and the handwritten Lean meaning) -----
...
def rows(abi=None):
    ...
    def ledger_of(shape):
        ...
        — computed independently of the decoder from the source_function's allocation structure and the host
    ...
    def add(source_function, args, expect, coverage, shape=None):
        ...
        out.append({
            "schema": SCHEMA, "id": f"{source_function}/{coverage}/{len(out)}",
            "source_function": source_function, "args": args, "expect": expect, "coverage": coverage,
        })
    ...
    for source_function in ("ssz_raw.readU32", "ssz_raw.readOffset"):
        add(source_function, {"data": pat8.hex(), "offset": 0}, ref_read_uint_le(pat8, 0, 4), "endian-pattern")
        add(source_function, {"data": pat8.hex(), "offset": 4}, ref_read_uint_le(pat8, 4, 4), "endian-pattern")
        add(source_function, {"data": (b"\x00" * 4).hex(), "offset": 0}, val_nat(0), "boundary-length")
        add(source_function, {"data": (b"\xff" * 4).hex(), "offset": 0}, val_nat(0xFFFFFFFF), "boundary-length")
        add(source_function, {"data": (b"\x01\x02\x03").hex(), "offset": 0}, INVALID, "offset-failure")  # 3 < 4
        add(source_function, {"data": pat8.hex(), "offset": 5}, INVALID, "offset-failure")               # 5+4>8
    ...
    for n in (20, 32, 48, 65, 96, 256):
        source_function = f"ssz_raw.readArray[{n}]"
        body = bytes((i * 7 + 3) & 0xFF for i in range(n))
        add(source_function, {"width": n, "data": body.hex(), "offset": 0}, ref_read_array(body, 0, n), "boundary-length")
        add(source_function, {"width": n, "data": (body + b"XY").hex(), "offset": 2},
            ref_read_array(body + b"XY", 2, n), "boundary-length")             # exact fit at offset 2
        add(source_function, {"width": n, "data": body[:-1].hex(), "offset": 0}, INVALID, "offset-failure")  # N-1 < N
    ...
    out.append({
        ...
        "source_function": "ssz_raw.requireU32Length", "args": {"data_len_gt_u32": True},
    ...
    def add_coll(source_function, elems, coverage, extra=None):
        body = b"".join(e[0] for e in elems)
        args = {"data": body.hex()}
        if extra:
            args.update(extra)
        # A collection allocates one block of `len(elems)` elements (none when empty).
        add(source_function, args, val_items([e[1] for e in elems]), coverage,
            shape=[(coll_elem_type[source_function], len(elems))])
...
def emit_lean(all_rows) -> str:
    ...
    for r in all_rows:
        if r["expect"]["kind"] == "gap":
            continue
        rt, a, e = r["source_function"], r["args"], r["expect"]
    ...
    L = [
        ...
        "end BinaryFv.Zesu.Validation.GeneratedSourceFunctionVectors",
    ]
    return "\n".join(L) + "\n"
