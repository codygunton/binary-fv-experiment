...
The host probe calls source functions from the pinned Zig decoder directly. Its job is to test the
handwritten contract meanings before those meanings are used in machine-code proofs.

Most decoder functions are private to their Zig source files. The validation-only
...
per-source-function examples. For allocating functions it records every allocation's order, size, alignment,
