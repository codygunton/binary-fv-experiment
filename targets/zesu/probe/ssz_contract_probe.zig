...
//! - call each cataloged source source_function with typed vectors and compare its exact value, error, and
...
//! every expected source_function is actually reachable by the probe.
//!
...
// Compile-time proof that the overlay exposes every private catalog source_function: referencing each
// `probe_*` re-export forces the compiler to resolve it against the private `fn` in the overlaid
// pinned source. If the overlay were missing (or a source_function renamed) this executable would not build.
comptime {
...
//
// This one allocator is used by BOTH the whole-input `raw.decode` ledger and the per-source_function
// allocating vectors (collections / containers / entry), so every allocating source_function shares one
...
fn emitOutcome(w: *Writer, id: []const u8, outcome: Outcome, error_name: []const u8) !void {
    ...
    try w.writeAll(",\"source_function\":\"ssz_raw.decode\",\"outcome\":");
...
/// The per-allocation event array in the final ledger format. Reused by the per-source_function allocating
/// vectors so every allocating source_function emits the same event shape.
...
fn emitLedger(w: *Writer, id: []const u8, outcome: Outcome, events: []const AllocEvent, leaked: bool, sweep: OomSweep) !void {
    try w.writeAll("{\"id\":");
    try writeJsonString(w, id);
    try w.print(",\"source_function\":\"ssz_raw.decode\",\"outcome\":\"{s}\",\"allocations\":{d}," ++
...
fn processLine(
    ...
    const source_function = jsonStr(obj, "source_function") orelse return false;
    if (!std.mem.eql(u8, source_function, "ssz_raw.decode")) return false;
...
// ============================================================================================
// Item 2/3: per-source_function typed vectors. Call each private catalog source_function (via the overlay) with the
...
/// The actual outcome of a source_function call, in the vector's value encoding (no host pointers).
...
// by a 0/1 presence bit. The order MUST match ssz_source_function_vectors.py's flat_fa/flat_fc/flat_cc and the
...
/// Dispatch one vector to its source_function. `input` is the decoded `args.data`; `arrbuf` backs readArray
/// slices and `scalarbuf` backs flattened container values (both must outlive this call).
fn runSourceFunction(source_function: []const u8, args: std.json.ObjectMap, input: []const u8, arrbuf: *[256]u8, scalarbuf: *[16]u64) ?Actual {
    const off: usize = @intCast(jsonInt(args, "offset") orelse 0);
    if (std.mem.eql(u8, source_function, "ssz_raw.readU32")) {
        if (raw.probe_readU32(input, off)) |v| return .{ .nat = v } else |e| return .{ .err = errLabel(@errorName(e)) };
    } else if (std.mem.eql(u8, source_function, "ssz_raw.readOffset")) {
        if (raw.probe_readOffset(input, off)) |v| return .{ .nat = v } else |e| return .{ .err = errLabel(@errorName(e)) };
    } else if (std.mem.eql(u8, source_function, "ssz_raw.readU64")) {
        if (raw.probe_readU64(input, off)) |v| return .{ .nat = v } else |e| return .{ .err = errLabel(@errorName(e)) };
    } else if (std.mem.eql(u8, source_function, "ssz_raw.readU256")) {
        if (raw.probe_readU256(input, off)) |v| return .{ .nat = v } else |e| return .{ .err = errLabel(@errorName(e)) };
    } else if (std.mem.eql(u8, source_function, "ssz_raw.bytesAt")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.requireU32Length")) {
        if (raw.probe_requireU32Length(input)) |_| return .{ .ok = {} } else |e| return .{ .err = errLabel(@errorName(e)) };
    } else if (std.mem.eql(u8, source_function, "ssz_raw.requireCanonicalOffsets")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.hasExactErePrefix")) {
        return .{ .boolean = raw.probe_hasExactErePrefix(input) };
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeOptionalU64")) {
        if (raw.probe_decodeOptionalU64(input)) |v| return .{ .opt_u64 = v } else |e| return .{ .err = errLabel(@errorName(e)) };
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeOptionalBlobSchedule")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeForkActivation")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeForkConfig")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeChainConfig")) {
    ...
    } else if (std.mem.startsWith(u8, source_function, "ssz_raw.readArray[")) {
    ...
    return null; // source_function not handled by this probe group
}
...
fn emitSourceFunctionOutcome(w: *Writer, id: []const u8, source_function: []const u8, actual: Actual, matched: bool) !void {
    try w.writeAll("{\"id\":");
    try writeJsonString(w, id);
    try w.writeAll(",\"source_function\":");
    try writeJsonString(w, source_function);
    switch (actual) {
...
/// An allocating source_function's outcome in the vector's value encoding. `items` is the flat collection
...
fn isCollectionSourceFunction(source_function: []const u8) bool {
    ...
    for (names) |n| if (std.mem.eql(u8, source_function, n)) return true;
    return false;
}
...
fn runCollection(source_function: []const u8, args: std.json.ObjectMap, input: []const u8, alloc: std.mem.Allocator, arena: std.mem.Allocator) ?AllocActual {
    const tb = TokBuilder{ .arena = arena };
    if (std.mem.eql(u8, source_function, "ssz_raw.decodeVersionedHashes")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodePublicKeys")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeWithdrawals")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeDepositRequests")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeWithdrawalRequests")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeConsolidationRequests")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeByteListList")) {
...
// exact field order of the pinned source. Element/leaf order matches ssz_source_function_vectors.py and Lean. --
...
fn isContainerSourceFunction(source_function: []const u8) bool {
    const names = [_][]const u8{
        "ssz_raw.decodeExecutionRequests", "ssz_raw.decodeExecutionWitness",
...
fn runContainer(source_function: []const u8, args: std.json.ObjectMap, input: []const u8, alloc: std.mem.Allocator, arena: std.mem.Allocator) ?AllocActual {
    _ = args;
    const tb = TreeBuilder{ .arena = arena };
    if (std.mem.eql(u8, source_function, "ssz_raw.decodeExecutionRequests")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeExecutionWitness")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeExecutionPayload")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeNewPayloadRequest")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decodeRaw")) {
    ...
    } else if (std.mem.eql(u8, source_function, "ssz_raw.decode")) {
...
fn emitAllocOutcome(w: *Writer, id: []const u8, source_function: []const u8, actual: AllocActual, matched: bool, events: []const AllocEvent, leaked: bool, sweep: OomSweep, ledger_match: bool) !void {
    try w.writeAll("{\"id\":");
    try writeJsonString(w, id);
    try w.writeAll(",\"source_function\":");
    try writeJsonString(w, source_function);
...
/// A runner decodes one allocating source_function under `alloc`, renders its value into `arena`, then frees.
...
const AllocRunner = *const fn (source_function: []const u8, args: std.json.ObjectMap, input: []const u8, alloc: std.mem.Allocator, arena: std.mem.Allocator) ?AllocActual;

/// Run one allocating source_function under a recording allocator failing at `fail_at`; classify for the sweep.
fn allocInjectOnce(gpa: std.mem.Allocator, run: AllocRunner, source_function: []const u8, args: std.json.ObjectMap, input: []const u8, fail_at: usize) []const u8 {
    var debug: std.heap.DebugAllocator(.{}) = .init;
    var rec = Recording.init(debug.allocator(), gpa, fail_at);
    var scratch = std.heap.ArenaAllocator.init(gpa);
    var reason: []const u8 = "";
    if (run(source_function, args, input, rec.allocator(), scratch.allocator())) |actual| {
        switch (actual) {
...
fn allocOomSweep(gpa: std.mem.Allocator, run: AllocRunner, source_function: []const u8, args: std.json.ObjectMap, input: []const u8, allocations: usize, max_inject: usize) OomSweep {
    if (allocations == 0) return .{ .safe = true, .injected = 0, .sampled = false, .reason = "" };
    const sampled = allocations > max_inject;
    const stride = if (sampled) (allocations + max_inject - 1) / max_inject else 1;
    var injected: usize = 0;
    var k: usize = 0;
    while (k < allocations) : (k += stride) {
        const reason = allocInjectOnce(gpa, run, source_function, args, input, k);
        injected += 1;
        if (reason.len != 0) return .{ .safe = false, .injected = injected, .sampled = sampled, .reason = reason };
    }
    return .{ .safe = true, .injected = injected, .sampled = sampled, .reason = "" };
...
/// Process one allocating-source_function vector (collection or container): value match + per-event allocation
...
/// the generator from the source_function's allocation structure + the host ABI table, not from the decoder).
...
fn processAllocLine(gpa: std.mem.Allocator, run: AllocRunner, out: *Writer, id: []const u8, source_function: []const u8, args: std.json.ObjectMap, input: []const u8, expect: std.json.ObjectMap) !bool {
    ...
    const actual = run(source_function, args, input, rec.allocator(), arena_inst.allocator()) orelse {
        _ = debug.deinit();
        try out.writeAll("{\"id\":");
        try writeJsonString(out, id);
        try out.writeAll(",\"source_function\":");
        try writeJsonString(out, source_function);
    ...
    const backing_leak = debug.deinit() == .leak;
    const leaked = self_leak or backing_leak;
    const sweep = allocOomSweep(gpa, run, source_function, args, input, allocations, default_max_inject);
    ...
    try emitAllocOutcome(out, id, source_function, actual, matched, rec.events.items, leaked, sweep, ledger_matched);
...
fn isRuntimeSourceFunction(source_function: []const u8) bool {
    const names = [_][]const u8{
        "raw_allocator.zesu_raw_alloc",     "raw_decoder_root.zesu_decode_raw",
        "raw_decoder_root.zesu_raw_result", "raw_decoder_root.zesu_raw_error",
        "raw_decoder_root.allocatorAlloc",  "raw_decoder_root.allocatorResize",
...
fn runRuntime(source_function: []const u8, args: std.json.ObjectMap, input: []const u8, gpa: std.mem.Allocator, arrbuf: *[256]u8) ?Actual {
    if (std.mem.eql(u8, source_function, "raw_allocator.zesu_raw_alloc")) {
    ...
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.allocatorAlloc")) {
        ZKVM_HEAP_POS = @intCast(jsonInt(args, "heap_pos") orelse 0);
        ZKVM_HEAP_TOP = @intCast(jsonInt(args, "heap_top") orelse 0);
        const bytes: usize = @intCast(jsonInt(args, "bytes") orelse 0);
        ...
        return .{ .err = "outOfMemory" };
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.allocatorResize")) {
        var ctx: u8 = 0;
        var mem = [_]u8{0} ** 8;
        return .{ .boolean = root_mod.probe_allocatorResize(@ptrCast(&ctx), mem[0..], @enumFromInt(0), 4, 0) };
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.allocatorRemap")) {
        var ctx: u8 = 0;
        var mem = [_]u8{0} ** 8;
        return .{ .boolean = root_mod.probe_allocatorRemap(@ptrCast(&ctx), mem[0..], @enumFromInt(0), 4, 0) != null };
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.allocatorFree")) {
        ...
        return .{ .ok = {} };
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.allocator")) {
        ...
        return .{ .ok = {} };
    } else if (std.mem.eql(u8, source_function, "memcpy") or std.mem.eql(u8, source_function, "memmove")) {
        const len: usize = @intCast(jsonInt(args, "len") orelse @as(i64, @intCast(input.len)));
        if (len > input.len or len > arrbuf.len) return null;
        const is_move = std.mem.eql(u8, source_function, "memmove");
        ...
        return .{ .bytes = arrbuf[0..len] };
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.zesu_decode_raw")) {
        const heap = gpa.alloc(u8, 1 << 20) catch return null;
        defer gpa.free(heap);
        return .{ .nat = @intCast(decodeUnderRoot(heap, input)) };
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.zesu_raw_error")) {
        const heap = gpa.alloc(u8, 1 << 20) catch return null;
        defer gpa.free(heap);
        _ = decodeUnderRoot(heap, input);
        return .{ .nat = root_mod.zesu_raw_error() };
    } else if (std.mem.eql(u8, source_function, "raw_decoder_root.zesu_raw_result")) {
        const heap = gpa.alloc(u8, 1 << 20) catch return null;
        defer gpa.free(heap);
        _ = decodeUnderRoot(heap, input);
...
/// Process one source_function-vector line; returns true if it was a defect (mismatch or unhandled source_function).
fn processSourceFunctionLine(gpa: std.mem.Allocator, out: *Writer, line: []const u8) !bool {
    var parsed = std.json.parseFromSlice(std.json.Value, gpa, line, .{}) catch return false;
    defer parsed.deinit();
    const obj = switch (parsed.value) {
        .object => |o| o,
    ...
    if (isCollectionSourceFunction(source_function)) return processAllocLine(gpa, runCollection, out, id, source_function, args, input, expect);
    if (isContainerSourceFunction(source_function)) return processAllocLine(gpa, runContainer, out, id, source_function, args, input, expect);
    ...
    if (isRuntimeSourceFunction(source_function)) {
        const actual = runRuntime(source_function, args, input, gpa, &arrbuf) orelse {
            try out.writeAll("{\"id\":");
            try writeJsonString(out, id);
            try out.writeAll(",\"source_function\":");
            try writeJsonString(out, source_function);
        ...
        try emitSourceFunctionOutcome(out, id, source_function, actual, matched);
        return !matched;
    }

    const actual = runSourceFunction(source_function, args, input, &arrbuf, &scalarbuf) orelse {
        // A vector naming a source_function this probe cannot reach/dispatch is a real gap in coverage.
        try out.writeAll("{\"id\":");
        try writeJsonString(out, id);
        try out.writeAll(",\"source_function\":");
        try writeJsonString(out, source_function);
...
fn runSourceFunctionVectors(init: std.process.Init, gpa: std.mem.Allocator, path: []const u8) !void {
    ...
    while (lines.next()) |line| {
        ...
        if (try processSourceFunctionLine(gpa, out, trimmed)) defects += 1;
    }
    try out.flush();
    if (defects != 0) {
        var msg_buf: [128]u8 = undefined;
        const msg = std.fmt.bufPrint(&msg_buf, "probe: {d} source_function vector(s) mismatched/unhandled\n", .{defects}) catch "probe: vector defects\n";
...
pub fn main(init: std.process.Init) !void {
    ...
    var source_function_vectors_path: ?[]const u8 = null;
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, a, "--list-routines")) {
            ...
            dump_abi = true;
        } else if (std.mem.eql(u8, a, "--source-function-vectors")) {
            i += 1;
            if (i >= args.len) fatal("--source-function-vectors needs a path");
            source_function_vectors_path = args[i];
    ...
    if (dump_abi) {
        ...
        // compute each source_function's expected allocation ledger independently of the decoder's own alloc.
    ...
    if (source_function_vectors_path) |rvp| {
        try runSourceFunctionVectors(init, gpa, rvp);
        return;
    }
