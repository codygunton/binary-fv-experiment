...
so its debug info describes the canonical bytes), identifies the single inline instance of
`ssz_raw.decodeOptionalBlobSchedule`, its inline call stack, and its nested child inline instances,
        ...
        if ln.strip().endswith("DW_TAG_inlined_subroutine"):
            ...
            if off and i + 1 < len(lines) and \
               ...
               "DW_AT_abstract_origin" in lines[i + 1]:
                return off.group(1)
    raise SystemExit("decodeOptionalBlobSchedule inline instance not found")
...
frames = []            # ordered: subprogram then inlined ancestors then the instance
i = 0
while i < len(plines):
...
# Instance subtree (ranges + immediate children).
...
for i, ln in enumerate(slines):
    ...
    if i + 1 < len(slines) and "DW_AT_abstract_origin" in slines[i + 1]:
        ...
        # only record the direct (shallowest) children of the instance
...
def emit_lean(d, src_hash="ea5a1b36f72c888a0bcb73f2ea1f2bf7ebf00c63c6460c84015d0f6783a1d131"):
    ...
    L.append("`tools/extract_blob_schedule_instance.py`. Object-relative DWARF ranges are")
    ...
    # primary instance identity
    L.append("/-- Address-free identity of the single inline occurrence of `decodeOptionalBlobSchedule`. -/")
    L.append(f'def blobScheduleInstanceId : InstanceId :=')
    L.append(f'  {{ function := {{ declaration := {sdecl(d["function"]["qualified"])}, specialization := #[] }},')
    L.append(f'    inlineStack := {stack(d["inlineStack"])} }}')
    L.append("")
    # parent (decodeForkConfig occurrence) identity — referenced only, not materialized
    L.append("/-- The enclosing occurrence (`decodeForkConfig`) this instance is inlined into. -/")
    L.append(f'def blobScheduleParentId : InstanceId :=')
    ...
    for n, c in enumerate(d["children"]):
        ...
        L.append(f'def readU64Field{n}Instance : FunctionInstance :=')
        L.append(f'  {{ id := readU64Field{n}Id, regions := {regions(rs)}, entryPc := {entry}, exitPcs := #[{last}],')
        L.append(f'    parent? := some blobScheduleInstanceId, children := #[], externalCalls := #[],')
    ...
    L.append("/-- The generated `decodeOptionalBlobSchedule` occurrence: three discontiguous canonical-ELF")
    L.append("    fragments, three nested `readU64` field reads, inlined into `decodeForkConfig`. -/")
    L.append("def blobScheduleInstance : FunctionInstance :=")
    L.append(f'  {{ id := blobScheduleInstanceId, regions := {regions(rs)}, entryPc := {entry}, exitPcs := #[{last}],')
    ...
    L.append(f'    provenance := {prov}, symbol? := none }}')
    L.append("")
    L.append("/-- The instance together with its nested children, as extracted. -/")
