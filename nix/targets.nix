  ...
  elflingProgram = pkgs.runCommand "elfling-program" {
    ...
    gen() {
      python3 ${elflingGeneratorScript} \
        ...
        --out-manifest "$1/GeneratedManifest.lean" \
        --out-manifest-md "$1/MANIFEST.md"
    }
    mkdir -p run1 run2 "$out"
    gen run1
    gen run2
    for f in program.json GeneratedProgram.lean program.md DecoderGlobals.lean GeneratedBindings.lean bindings.json GeneratedManifest.lean MANIFEST.md; do
      cmp -s "run1/$f" "run2/$f" \
    ...
    cp run1/program.json run1/GeneratedProgram.lean run1/program.md run1/DecoderGlobals.lean \
       run1/GeneratedBindings.lean run1/bindings.json run1/GeneratedManifest.lean \
       run1/MANIFEST.md "$out/"
  ...
  sszScaleEvidence =
    ...
    pkgs.runCommand "ssz-scale-function-instance-evidence" {
      ...
      python3 trace/static_reachability.py \
        ...
        --out-json "$out/static-reachability.json" --out-md "$out/STATIC_REACHABILITY.md"
      ...
      python3 trace/cfg_audit.py \
        --objdump ${riscvObjdump} --elf ${zesuSsz}/bin/zesu-ssz \
        --program ${elflingProgram}/program.json --out-json "$out/cfg-audit.json"
      python3 - "$out/cfg-audit.json" <<'PY'
      import json, sys
