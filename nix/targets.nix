...
let
  ...
  elflingGeneratorTest = builtins.path {
    path = repo + "/targets/zesu/tests/elfling_program_test.py";
  ...
  elflingProgram = pkgs.runCommand "elfling-program" {
    ...
    mkdir -p source/tools source/targets/zesu/tests
    cp ${elflingGeneratorScript} source/tools/generate_elfling_program.py
    cp ${elflingGeneratorTest} source/targets/zesu/tests/elfling_program_test.py
    python3 source/targets/zesu/tests/elfling_program_test.py
