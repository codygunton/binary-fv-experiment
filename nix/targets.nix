...
let
  ...
  sszScaleEvidence =
    ...
    pkgs.runCommand "ssz-scale-function-instance-evidence" {
      ...
      import json, sys
      s = json.load(open(sys.argv[1]))["summary"]
      print(f"scaled per-function_instance evidence: {s['covered']}/{s['function_instances']} function_instances covered; "
            "matches committed module")
      for n, d in sorted(s["byCheck"].items()):
          print(f"  {n:22s} pass={d['pass']:3d} fail={d['fail']:3d} gap={d['gap']:3d}")
      assert sum(d["fail"] for d in s["byCheck"].values()) == 0, "a per-function_instance check FAILED"
