...
The shared `Option` transports, `ByteWindowRelocation.atOffset`, `Region.mem_iUnion`, register/load
APIs, and owned routes are ordinary explicit lemmas, not candidates for new `simp` or `grind`
registrations. Reuse them directly; keep this document about automation sets and patterns.
