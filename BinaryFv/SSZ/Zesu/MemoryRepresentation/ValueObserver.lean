...
/-!
# Observing a complete `SszBridge.RawV4` value
...
needs: one guarded, executable `observeRawV4?` that reconstructs the bridge's own `SszBridge.RawV4`
...
def observeByteVector? (state : State) (base length : Nat) :
    Option (SszBridge.RawByteVector length) := do
...
def observeByteSlice? (state : State) (descriptorBase : Nat) : Option SszBridge.RawBytes := do
...
def observeWithdrawalValue? (state : State) (base : Nat) : Option SszBridge.RawWithdrawal := do
...
theorem observe_withdrawal_value_of_rep (state : State) (base : Nat)
    (value : SszBridge.RawWithdrawal) (representation : RawWithdrawalRep state base value) :
...
def observeDepositRequestValue? (state : State) (base : Nat) :
    Option SszBridge.RawDepositRequest := do
...
theorem observe_deposit_request_value_of_rep (state : State) (base : Nat)
    (value : SszBridge.RawDepositRequest)
...
def observeWithdrawalRequestValue? (state : State) (base : Nat) :
    Option SszBridge.RawWithdrawalRequest := do
  let amount ← observeUInt64? state base
...
def observeConsolidationRequestValue? (state : State) (base : Nat) :
    Option SszBridge.RawConsolidationRequest := do
...
/-- **One guarded observation of the complete `SszBridge.RawV4`.** Reconstructs every fixed scalar
...
def observeRawV4? (state : State) (rootBase : Nat) : Option SszBridge.RawV4 := do
