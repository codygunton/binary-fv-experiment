...
theorem returnExit_fetch_decode_base {nodes : Array ControlFlowNode}
    ...
    (state : State) (intact : Artifact.programImage.fileBytesMatchMemory state.mem)
  ...
  obtain ⟨byte0, byte1, byte2, byte3, hfetch, hword⟩ :=
    ProgramImage.fetchBytesAt_of_ownedFileEncodedWord Artifact.programImage state
...
structure ExitPlatform (state : State) (pc : Nat) : Prop where
  ...
  code : Artifact.programImage.fileBytesMatchMemory state.mem
...
`minstret` on every retirement, so preserving it would be false of every routine), and the code
...
theorem exitPlatform_of_agree {before after : State} {pc : Nat}
    ...
    (code : Artifact.programImage.fileBytesMatchMemory after.mem)
...
the runner's `PC := Artifact.zesuDecodeRaw` really enters the instance whose exit is pinned. -/
...
theorem buildZesuEntryState_exitPlatform (input : ByteArray) :
  ...
  have hcode : Artifact.programImage.fileBytesMatchMemory s.mem := by
    have h : canonicalEnvironment.image.fileBytesMatchMemory s.mem := hbind.2.1
    have himg : canonicalEnvironment.image = Artifact.programImage := by
