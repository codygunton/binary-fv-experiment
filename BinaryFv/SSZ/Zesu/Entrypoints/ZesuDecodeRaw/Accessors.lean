...
theorem rawV4Rep_accessorSetup {state : State} {inputBase rootBase : Nat} {input : ByteArray}
    {value : SszBridge.RawV4} (entryPc : Nat)
...
The witness is the builder's own state rather than one written to suit, for the same reason
`CatalogSatisfiability` uses it: `CodeIntact` demands roughly twenty kilobytes of file-backed image
bytes, which cannot be written down by hand. -/
theorem accessor_entry_bindings_satisfiable :
    ∃ (state : State) (model : DecoderGlobalsModel),
      (contractRawError canonicalEnvironment Elfling.canonicalDecoderGlobalsLayout).pre model
        (accessorSetup resolvedSymbols.rawError state) ∧
      (contractRawResult canonicalEnvironment Elfling.canonicalDecoderGlobalsLayout
        Elfling.canonicalResultBuffer).pre model
...
theorem storedResultDiscriminantRep_of_observation {model : DecoderGlobalsModel} {state : State}
    ...
    StoredResultDiscriminantRep Elfling.canonicalDecoderGlobalsLayout model state :=
...
theorem observeReturnCode_of_postRawError {env : DecoderEnvironment} {model : DecoderGlobalsModel}
    {result : Except SszDecodeError Nat} {before after : State} {code : Nat}
...
def AcceptedAccessorTraces (final : State) : Prop :=
  ∃ middle after : State,
    ...
    observeReturnCode? middle = some Elfling.canonicalResultBuffer ∧
...
theorem accessors_accepted_of_traces {final : State} (steps : Nat)
    ...
    ∃ after : State, Runs (runAccessorsIfReached resolvedSymbols (.reached steps)) final after
      (.returned Elfling.canonicalResultBuffer, .returned DecodeStatus.ok.code) := by
...
`AcceptedAccessorTraces`/`RejectedAccessorTraces` and nothing else. Whatever else the D5 scaffolds
still owe, the accessor half of them is now one named premise apiece. -/

/-- **A successful run, assembled.** Every field but the accessors comes from the decode; the
accessors come from the named residue. -/
theorem successfulRun_of_acceptedAccessorTraces (input : ByteArray) (value : SszBridge.RawV4)
...
theorem accepted_accessor_codes_differ :
    Elfling.canonicalResultBuffer ≠ DecodeStatus.ok.code := by native_decide
