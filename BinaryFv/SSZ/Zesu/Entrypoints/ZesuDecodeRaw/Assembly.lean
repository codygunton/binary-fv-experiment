...
What this module supplies is what none of the three could: the obligations themselves, out of
`LocalContractAssumptions` through the generated dispatch, and the three transports that carry a fact
...
`LocalContractAssumptions` and nothing else. Every theorem in this module takes it as its only
hypothesis beyond the input bound and the specification outcome.
-/
...
theorem rawV4Rep_of_mem_eq {s t : State} {inputBase rootBase : Nat} {input : ByteArray}
    {value : SszBridge.RawV4} (hmem : t.mem = s.mem)
  ...
  obtain ⟨_, htransport⟩ :=
    Footprint.rawV4_footprint_abi inputBase input rootBase value s
      BinaryFv.SSZ.Zesu.Artifact.raw_stateless_input_layout.1 h
...
the stack, which is a fact about two pinned address ranges rather than about the routine. -/
...
theorem decoderGlobals_below_ceiling :
    Elfling.canonicalDecoderGlobalsLayout.attempted < globalsCeiling ∧
      Elfling.canonicalDecoderGlobalsLayout.status + 3 < globalsCeiling ∧
...
theorem canonicalResultBuffer_below_ceiling : Elfling.canonicalResultBuffer < globalsCeiling := by
  native_decide
...
theorem decoderGlobalsScalarRep_survives_accessor {before after : State}
    ...
    (h : DecoderGlobalsScalarRep Elfling.canonicalDecoderGlobalsLayout model before) :
    DecoderGlobalsScalarRep Elfling.canonicalDecoderGlobalsLayout model after := by
  ...
  · intro index hindex
    rw [frame _ (globals_outside_accessor_permission
      (show Elfling.canonicalDecoderGlobalsLayout.status + index < globalsCeiling by
...
theorem storedResultDiscriminantRep_survives_accessor {before after : State}
    {model : DecoderGlobalsModel}
    (frame : canonicalEnvironment.WritesOnlyWithinOwnRecord 0 0 before after)
    (h : StoredResultDiscriminantRep Elfling.canonicalDecoderGlobalsLayout model before) :
    StoredResultDiscriminantRep Elfling.canonicalDecoderGlobalsLayout model after := by
  show after.mem.get? _ = _
...
theorem programImage_of_codeIntact {state : State} (h : canonicalEnvironment.CodeIntact state) :
    Artifact.programImage.fileBytesMatchMemory state.mem := by
  have himage : canonicalEnvironment.image = Artifact.programImage := by
...
/-! ## Each of the three instances' obligations, out of `LocalContractAssumptions`

`LocalContractAssumptions` is the per-function-instance *local* obligation;
`sszProgramCorrectness_of_locals` composes it into the closed one for every instance, and
`function_instance_implements_its_contract` selects the instance. What each selection still needs is
the tag — which contract this instance owes — and that is read through the address-free catalog by
the three `*_function_instance_tag` theorems, never chosen here. -/

/-- Program correctness at the canonical parameters, from the local proofs alone. -/
theorem canonical_program_correctness (locals : LocalContractAssumptions) :
    sszProgramCorrectness generatedProgram canonicalContractParams :=
  sszProgramCorrectness_of_locals catalogSemanticObligations_holds
    canonical_catalog_satisfiability locals

/-- **The exported wrapper implements `functionInstanceZesuDecodeRaw`.** This is the hypothesis
`canonicalDecodeExit_of_implements` consumes, and it is where the whole value half enters. -/
theorem entry_implements_of_locals (locals : LocalContractAssumptions) {fi : FunctionInstance}
    ...
    BinaryFv.RiscV.Elfling.FunctionInstanceContract.Implements
      (functionInstanceExecutionPcs generatedProgram fi)
      (functionInstanceExitPred fi) (functionInstanceEntryWord fi)
      (functionInstanceZesuDecodeRaw canonicalContractParams.env canonicalContractParams.globals
        ...
        DecoderGlobalsModel.fresh) := by
  obtain ⟨entry, hentry, htag⟩ :=
    Option.map_eq_some_iff.mp (entry_function_instance_tag fi hfind)
  have h := function_instance_implements_its_contract (canonical_program_correctness locals)
    (entry_function_instance_mem hfind) hentry
  rw [htag] at h
  exact h

/-- **`zesu_raw_result` implements `contractRawResult`.** -/
theorem rawResult_implements_of_locals (locals : LocalContractAssumptions) {fi : FunctionInstance}
    (hmem : fi ∈ generatedProgram.functionInstances)
    (hentry : fi.entryPc = resolvedSymbols.rawResult) :
    BinaryFv.RiscV.Elfling.Implements (functionInstanceExecutionPcs generatedProgram fi)
      (functionInstanceExitPred fi) (functionInstanceEntryWord fi)
      (contractRawResult canonicalContractParams.env canonicalContractParams.globals
        canonicalContractParams.resultBuffer) := by
  obtain ⟨entry, hentry', htag⟩ :=
    Option.map_eq_some_iff.mp (rawResult_function_instance_tag hmem hentry)
  have h := function_instance_implements_its_contract (canonical_program_correctness locals) hmem
    hentry'
  rw [htag] at h
  exact h

/-- **`zesu_raw_error` implements `contractRawError`.** -/
theorem rawError_implements_of_locals (locals : LocalContractAssumptions) {fi : FunctionInstance}
    (hmem : fi ∈ generatedProgram.functionInstances)
    (hentry : fi.entryPc = resolvedSymbols.rawError) :
    BinaryFv.RiscV.Elfling.Implements (functionInstanceExecutionPcs generatedProgram fi)
      (functionInstanceExitPred fi) (functionInstanceEntryWord fi)
      (contractRawError canonicalContractParams.env canonicalContractParams.globals) := by
  obtain ⟨entry, hentry', htag⟩ :=
    Option.map_eq_some_iff.mp (rawError_function_instance_tag hmem hentry)
  have h := function_instance_implements_its_contract (canonical_program_correctness locals) hmem
    hentry'
  rw [htag] at h
  exact h
...
theorem accessorTraces_of_exported (contracts : ExportedContractAssumptions) {state : State}
    ...
    (hstored : StoredResultDiscriminantRep Elfling.canonicalDecoderGlobalsLayout model state) :
    ∃ middle after : State,
      AccessorReachesSentinel resolvedSymbols.rawResult rawResultStepBound state middle ∧
      observeReturnCode? middle
        = some (if model.stored.isSome then Elfling.canonicalResultBuffer else 0) ∧
  ...
  have hpointerBound :
      (if model.stored.isSome then Elfling.canonicalResultBuffer else 0) < 2 ^ 64 := by
  ...
  have hscalarMiddle : DecoderGlobalsScalarRep Elfling.canonicalDecoderGlobalsLayout model middle :=
...
theorem decodeRun_of_exported (contracts : ExportedContractAssumptions) (input : ByteArray) :
  ...
  have hcodeEntry : Artifact.programImage.fileBytesMatchMemory entryState.mem :=
...
theorem successfulRun_of_exported (contracts : ExportedContractAssumptions) {input : ByteArray}
  ...
  have hvalueFinal : RawV4Rep finalState canonicalRunnerLayout.inputBase input
      Elfling.canonicalResultBuffer value := rawV4Rep_of_mem_eq hframe.mem hvalue
