...
theorem heapPos_not_file_backed :
    Artifact.programImage.readFileByte? zkvmHeapPos = none := by native_decide

/-- The `decodeRaw` entry instruction is a real file-backed code byte. -/
theorem codeByte_file_backed :
    (Artifact.programImage.readFileByte? 0x10444).isSome = true := by native_decide
...
theorem codeIntact_insert_heapPos {mem : Std.ExtHashMap Nat (BitVec 8)} {value : BitVec 8}
    (h : Artifact.programImage.fileBytesMatchMemory mem) :
    Artifact.programImage.fileBytesMatchMemory (mem.insert zkvmHeapPos value) :=
...
theorem not_codeIntact_corrupt_code {mem : Std.ExtHashMap Nat (BitVec 8)} :
    ∃ value : BitVec 8, ¬ Artifact.programImage.fileBytesMatchMemory (mem.insert 0x10444 value) := by
