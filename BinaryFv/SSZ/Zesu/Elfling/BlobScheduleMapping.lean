...
function instance and its generated entry/exit. Stated here; its semantic discharge (both option branches,
live-state instantiation, stores) is row 3 (`ssz-elfling-blob-schedule`). This is the join the whole
Elfling layering exists to make: an address-free claim, every address supplied by the function instance. -/
def blobScheduleCorrectnessObligation (env : DecoderEnvironment) : Prop :=
  correctnessClaimOptionalBlobSchedule env blobScheduleFunctionInstance blobScheduleEntry blobScheduleExit
