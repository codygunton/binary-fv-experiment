...
This module validates the generated `decodeOptionalBlobSchedule` function instance
(`BlobScheduleFunctionInstance.lean`, extracted from the DWARF sidecar and mapped to canonical-ELF PCs)
...
1. **Identity.** Through the *address-free* catalog (`catalogEntryFor`), the generated function instance's
   identity resolves to exactly the `decodeOptionalBlobSchedule` routine, so it is the function instance the
...
2. **Coverage.** Every PC of the existing 66-step trace over `0x12cbc..0x12dc0`
   (`BlobScheduleAndResultStores`) lies inside the function instance's three discontiguous fragments.
3. **Attribution.** The three nested `readU64` field-read function instances are properly nested in the
...
/-- The three nested `readU64` field-read function instances. -/
def blobScheduleChildren : Array FunctionInstance :=
  #[readU64Field0FunctionInstance, readU64Field1FunctionInstance, readU64Field2FunctionInstance]
...
/-- The generated function instance's identity resolves, through the address-free catalog, to exactly the
...
theorem blobSchedule_identity_matches_catalog :
    (catalogEntryFor blobScheduleFunctionInstance.id.function).map (·.tag)
      = some RoutineTag.optionalBlobSchedule := by
  native_decide

/-! ## 2. Coverage — the 66-step trace is covered by the function instance -/

/-- Every PC of the existing 66-step trace lies inside one of the function instance's three fragments. -/
theorem blobSchedule_trace_covered :
    blobScheduleTraceSites.all (fun pc => blobScheduleFunctionInstance.containsAddress pc) = true := by
  native_decide
...
/-- `range` is contained in some region of `functionInstance`. -/
...
/-- Each nested `readU64` function instance is contained in the parent function instance's regions. -/
theorem blobSchedule_children_nested :
    blobScheduleChildren.all (fun c => functionInstanceNestedIn c blobScheduleFunctionInstance) = true := by
  native_decide

/-- The three sibling `readU64` function instances are pairwise disjoint: no overlapping ownership. -/
theorem blobSchedule_children_disjoint :
    (functionInstancesDisjoint readU64Field0FunctionInstance readU64Field1FunctionInstance &&
     functionInstancesDisjoint readU64Field0FunctionInstance readU64Field2FunctionInstance &&
     functionInstancesDisjoint readU64Field1FunctionInstance readU64Field2FunctionInstance) = true := by
  native_decide
...
/-- The first PC (if any) at which two function instances' regions overlap. -/
...
/-- The attribution defects for the blob-schedule function instance, **computed** from its extracted data:
...
def blobScheduleAttributionDefects : Array AttributionDefect :=
  nestingDefects blobScheduleFunctionInstance blobScheduleChildren ++
...
owned by a `readU64` child, none directly by `decodeOptionalBlobSchedule`. So the function instance's own
...
/-- Every instruction PC (4-byte stride) the function instance's three fragments claim. -/
def blobScheduleFunctionInstancePCs : Array Nat :=
  blobScheduleFunctionInstance.regions.flatMap fun r =>
    (List.range (r.size / 4)).toArray.map (fun k => r.start + 4 * k)

/-- **Every** function instance PC the 66-step exemplar trace does not exercise, at instruction granularity —
...
def blobScheduleUnexercisedPCs : Array Nat :=
  blobScheduleFunctionInstancePCs.filter (fun pc => !blobScheduleTraceSites.contains pc)

/-- Nothing is dropped: every function instance PC is either exercised by the trace or surfaced as
unexercised. -/
theorem blobSchedule_pc_partition :
    blobScheduleFunctionInstancePCs.all
...
/-! ## 5. Contract binding — the generated function instance implements the handwritten contract -/

/-- The function instance's entry PC as a machine word. -/
def blobScheduleEntry : BitVec 64 := BitVec.ofNat 64 blobScheduleFunctionInstance.entryPc

/-- The function instance's exit predicate: exactly its generated exit PCs. -/
def blobScheduleExit (pc : BitVec 64) : Prop := pc.toNat ∈ blobScheduleFunctionInstance.exitPcs

/-- The extracted slice as a `Program` in its own right: the function instance together with the three
nested `readU64` children it inlines. The function instance's execution extent is then computed from the
same generated data the whole-program obligation uses, rather than being written by hand. -/
def blobScheduleProgram : Program :=
  { entry := blobScheduleFunctionInstanceId, functionInstances := blobScheduleFunctionInstances, defects := #[],
    provenance := blobScheduleFunctionInstance.provenance }

/-- The address-free `decodeOptionalBlobSchedule` correctness obligation, bound to the generated
function instance and its generated entry/exit/extent. Stated here; its semantic discharge (both option
...
def blobScheduleCorrectnessObligation (env : DecoderEnvironment) : Prop :=
  correctnessClaimOptionalBlobSchedule env blobScheduleFunctionInstance
    (Contracts.functionInstanceReachedPcs blobScheduleProgram blobScheduleFunctionInstance) blobScheduleEntry
    blobScheduleExit
