...
read (`Generated.generatedSourceManifest`), and that every occurrence's recorded declaration line is
the routine's DWARF-resolved declaration (`Generated.generatedDeclLines`), not merely positive.

Together with `sourceProvenanceRecorded` (which already checks each occurrence's `sourceFileHash` for
*equality* with `pinnedSourceManifest`), this ties: handwritten manifest = generated manifest, each
occurrence hash = manifest entry, and each occurrence declaration line = the resolved declaration —
...
/-! ## 2. Every occurrence's declaration line matches its routine's resolved declaration -/

/-- Whether an occurrence's declaration line equals the DWARF-resolved declaration line recorded for
its routine in `generatedDeclLines`. -/
def occurrenceDeclResolved (inst : FunctionInstance) : Bool :=
  generatedDeclLines.contains
    (inst.id.function.declaration.qualifiedName, inst.declProvenance.declSpan.line)

/-- **Every generated occurrence's `declSpan` line is its routine's resolved declaration line.** Since
`generatedDeclLines` carries one line per routine (and the generator surfaces a defect if two
occurrences of a routine disagree), this is the "declaration span matches the resolved source
declaration" check the review asked for — strictly stronger than `declSpan.line > 0`. -/
theorem every_occurrence_declSpan_matches_resolved :
    generatedProgram.instances.all occurrenceDeclResolved = true := by native_decide
