...
generated `children`/`externalCalls`, whose resolution is checked by `calleesResolveB` and whose
statically decodable transfers are accounted by `GeneratedBoundaryInventory`). What it does detect is
...
theorem executionExtentReadableIn_elim {image : ProgramImage} {program : Program}
  ...
  obtain ⟨r, hr, hlo, hhi⟩ := executionPcs_iff_ranges.mp hpc
