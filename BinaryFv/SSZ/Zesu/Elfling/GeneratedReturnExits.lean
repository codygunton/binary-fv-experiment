...
`Artifact.programImage` leaves the read symbolic. -/
...
theorem returnExitsAreRet_check :
    ∀ nodes, controlFlow? = some nodes →
      returnExitsAreRetB nodes Artifact.programImage generatedProgram = true := by
  intro nodes hn
  have : (controlFlow?.map
      (fun ns => returnExitsAreRetB ns Artifact.programImage generatedProgram)).getD false = true := by
    native_decide
...
theorem adding_non_ra_return_exit_breaks_check :
    nonRaReturnSitePcs.all
      (fun a => (controlFlow?.map fun nodes =>
        returnExitsAreRetB nodes Artifact.programImage (programWithExtraExit a)).getD true == false)
      = true := by native_decide
...
theorem adding_non_return_exit_does_not_break_check :
    (controlFlow?.map fun nodes =>
      returnExitsAreRetB nodes Artifact.programImage (programWithExtraExit someNonReturnPc)).getD
