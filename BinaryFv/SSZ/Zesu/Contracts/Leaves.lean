...
def meaningReadU256 (bytes : ByteArray) (offset : Nat) : Except SszDecodeError (BitVec 256) :=
  ...
  | .ok slice =>
      ...
      -- earlier form read `slice[31 - index]`, which is big-endian and contradicts both this routine's
      -- `.little` source and the little-endian `BitVectorLERep` post-condition; the Row B per-routine
...
def correctnessClaimReadU32 (env : DecoderEnvironment)
  ...
  ImplementsInstance instance_ reached entry exit (contractReadU32 env)

def correctnessClaimReadU64 (env : DecoderEnvironment)
    (instance_ : BinaryFv.Binary.Elfling.FunctionInstance) (reached : BitVec 64 → Prop)
    (entry : BitVec 64) (exit : BitVec 64 → Prop) : Prop :=
  ImplementsInstance instance_ reached entry exit (contractReadU64 env)

def correctnessClaimReadOffset (env : DecoderEnvironment)
    (instance_ : BinaryFv.Binary.Elfling.FunctionInstance) (reached : BitVec 64 → Prop)
    (entry : BitVec 64) (exit : BitVec 64 → Prop) : Prop :=
  ImplementsInstance instance_ reached entry exit (contractReadOffset env)

def correctnessClaimHasExactErePrefix (env : DecoderEnvironment)
    (instance_ : BinaryFv.Binary.Elfling.FunctionInstance) (reached : BitVec 64 → Prop)
    (entry : BitVec 64) (exit : BitVec 64 → Prop) : Prop :=
  ImplementsInstance instance_ reached entry exit (contractHasExactErePrefix env)

def correctnessClaimRequireU32Length (env : DecoderEnvironment)
    (instance_ : BinaryFv.Binary.Elfling.FunctionInstance) (reached : BitVec 64 → Prop)
    (entry : BitVec 64) (exit : BitVec 64 → Prop) : Prop :=
  ImplementsInstance instance_ reached entry exit (contractRequireU32Length env)

def correctnessClaimBytesAt (env : DecoderEnvironment)
    (instance_ : BinaryFv.Binary.Elfling.FunctionInstance) (reached : BitVec 64 → Prop)
    (entry : BitVec 64) (exit : BitVec 64 → Prop) : Prop :=
  ImplementsInstance instance_ reached entry exit (contractBytesAt env)

def correctnessClaimReadU256 (env : DecoderEnvironment)
    (instance_ : BinaryFv.Binary.Elfling.FunctionInstance) (reached : BitVec 64 → Prop)
    (entry : BitVec 64) (exit : BitVec 64 → Prop) : Prop :=
  ImplementsInstance instance_ reached entry exit (contractReadU256 env)

/-- One correctness claim per concrete `readArray` width. -/
def correctnessClaimReadArray (env : DecoderEnvironment) (length : Nat)
    (instance_ : BinaryFv.Binary.Elfling.FunctionInstance) (reached : BitVec 64 → Prop)
    (entry : BitVec 64) (exit : BitVec 64 → Prop) : Prop :=
  ImplementsInstance instance_ reached entry exit (contractReadArray env length)
