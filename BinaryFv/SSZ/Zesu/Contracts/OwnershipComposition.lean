...
`DecoderEnvironment.WritesOnlyWithinOwnRecord` for the non-allocating routines,
...
in.** First: an allocating routine was said to "write nowhere outside those two regions", but a bump
allocator advances `ZKVM_HEAP_POS`, which is in neither, so the two-region clause was false of every
allocating routine in the decoder. Then: *every* compiled routine writes its stack frame, which is in
none of the three, so the three-region clause was false of every routine whatsoever.
...
that point the clause is not strong but false, and since these are consumed only through an assumed
`LocalContractAssumptions`, a false conjunct is not a failed proof — it is a vacuous root that reads
as a stronger one. `stack_writing_routine_satisfies_the_clause` below is the standing check that the
current form has not crossed that line. -/
...
/-- A **non-allocating** routine's step: its record and its stack frame. -/
...
/-- The allocating routine's step: `DecoderEnvironment.ownedRegion` — record, arena interval,
...
/-! ## The clause is satisfiable by a routine that touches its stack frame

**The obligation this whole layer stands or falls on.** The clause is consumed only through the
assumed `LocalContractAssumptions`, so if no compiled routine could satisfy it the hypothesis would
be *false* and every downstream theorem vacuously true while reading as though it had been
...
Real, here, means all three at once: the routine writes its result record, it advances the bump
...
inside the arena interval the routine consumed, the cursor's second byte at `901` (advancing it from
...
/-- Every byte of the cursor word except the one the routine advanced is still zero. -/
...
/-- **The routine allocated:** the cursor stands at `256` afterwards, so the interval `[0, 256)` is
what it consumed. -/
...
/-- **A routine shaped like a real one satisfies the strengthened clause, and refutes the previous
one.**

The run: the record byte at `100` goes `0 → 3`; the cursor at `900` advances `0 → 256` and the
routine writes at `220`, inside the interval it consumed; and it writes `1016`, in its stack frame.
...
* **`contractAllocatorFree` was read as needing nothing, and that reading was wrong.** It stated an
  unrestricted total memory frame, which does imply the clause for every region — but only a routine
  ...
  now carries `env.WritesOnlyWithinOwnRecord 0 0` like the other no-op routines.
