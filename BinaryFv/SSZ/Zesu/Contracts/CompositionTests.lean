...
`global_of_local` combines local function instance proofs into closed machine traces. These examples show
...
* **Circular dependency.** A pair of function instances that call each other, and a function instance that calls
...
about which routine a function instance implements — they are about the shape of the dependency graph and
...
private abbrev testId (name : String) : FunctionInstanceId :=
  { function := { declaration := { file := { path := "test.zig" }, qualifiedName := name }
                  specialization := #[] }
    inlineStack := [] }

private abbrev testFunctionInstance (name : String) (start size entryPc : Nat) (exitPcs : Array Nat)
    (children : Array FunctionInstanceId) : FunctionInstance :=
...
private abbrev testProgram (functionInstances : Array FunctionInstance) (entry : FunctionInstanceId) : Program :=
  { entry := entry, functionInstances := functionInstances, defects := #[], provenance := testProvenance }

/-! ## 1. Circular dependencies admit no rank -/

/-- `a` and `b` call each other. -/
abbrev mutualA : FunctionInstance := testFunctionInstance "a" 100 100 100 #[196] #[testId "b"]
abbrev mutualB : FunctionInstance := testFunctionInstance "b" 200 100 200 #[296] #[testId "a"]

abbrev mutualProgram : Program := testProgram #[mutualA, mutualB] (testId "a")

private theorem mutualA_mem : mutualA ∈ mutualProgram.functionInstances :=
  Array.mem_iff_getElem.mpr ⟨0, by decide, rfl⟩

private theorem mutualB_mem : mutualB ∈ mutualProgram.functionInstances :=
  Array.mem_iff_getElem.mpr ⟨1, by decide, rfl⟩

private theorem mutualB_callee : mutualB ∈ calleeFunctionInstances mutualProgram mutualA :=
  Array.mem_filter.mpr ⟨mutualB_mem, by simp⟩

private theorem mutualA_callee : mutualA ∈ calleeFunctionInstances mutualProgram mutualB :=
  Array.mem_filter.mpr ⟨mutualA_mem, by simp⟩

/-- **Mutual recursion has no rank.** Each of the two function instances would have to rank strictly below
...
/-! ## 2. A function instance in its own dependency set admits no rank -/

/-- A single function instance naming itself as an inline child. -/
abbrev selfCaller : FunctionInstance := testFunctionInstance "self" 100 100 100 #[196] #[testId "self"]

abbrev selfProgram : Program := testProgram #[selfCaller] (testId "self")

private theorem selfCaller_mem : selfCaller ∈ selfProgram.functionInstances :=
  Array.mem_iff_getElem.mpr ⟨0, by decide, rfl⟩

private theorem selfCaller_callee : selfCaller ∈ calleeFunctionInstances selfProgram selfCaller :=
  Array.mem_filter.mpr ⟨selfCaller_mem, by simp⟩

/-- **A function instance may not occur in its own dependency set.** If it does, it would have to rank
...
abbrev caller : FunctionInstance := testFunctionInstance "caller" 100 100 100 #[196] #[testId "leaf"]
abbrev leaf : FunctionInstance := testFunctionInstance "leaf" 200 100 200 #[296] #[]

abbrev rankedProgram : Program := testProgram #[caller, leaf] (testId "caller")

private theorem caller_mem : caller ∈ rankedProgram.functionInstances :=
  Array.mem_iff_getElem.mpr ⟨0, by decide, rfl⟩

private theorem leaf_mem : leaf ∈ rankedProgram.functionInstances :=
  Array.mem_iff_getElem.mpr ⟨1, by decide, rfl⟩

private theorem leaf_callee : leaf ∈ calleeFunctionInstances rankedProgram caller :=
  Array.mem_filter.mpr ⟨leaf_mem, by simp⟩

/-- The honest rank: the callee below its caller. -/
def goodRank (functionInstance : FunctionInstance) : Nat :=
  if functionInstance.entryPc = 100 then 1 else 0

/-- The inverted rank: the caller below its callee. -/
def invertedRank (functionInstance : FunctionInstance) : Nat :=
  if functionInstance.entryPc = 100 then 0 else 1
...
theorem negative_missing_child_summary
    {p : ContractParams} {program : Program} {functionInstance callee : FunctionInstance}
    {entry : CatalogEntry}
    (hcallee : callee ∈ calleeFunctionInstances program functionInstance)
    ...
    (available : ChildSummariesAvailable p program functionInstance (fun _ _ _ _ _ => False)) :
    False := by
...
theorem childSummariesAvailable_not_vacuous
    {p : ContractParams} {program : Program} {functionInstance callee : FunctionInstance}
    {entry : CatalogEntry}
    {childSummary : FunctionInstanceId → Nat → Nat → BinaryFv.RiscV.State → BinaryFv.RiscV.State → Prop}
    (hcallee : callee ∈ calleeFunctionInstances program functionInstance)
    (found : catalogEntryFor callee.id.function = some entry)
    (satisfiable : ∃ (args : (routineContract p callee.id.function entry.tag).Args) (s : BinaryFv.RiscV.State),
      (routineContract p callee.id.function entry.tag).contract.binding.entry args s)
    (available : ChildSummariesAvailable p program functionInstance childSummary) :
...
return. -/
abbrev boundaryChild : FunctionInstance := testFunctionInstance "child" 140 20 140 #[156] #[]

abbrev boundaryParent : FunctionInstance :=
  testFunctionInstance "parent" 100 100 100 #[150] #[testId "child"]

abbrev boundaryProgram : Program := testProgram #[boundaryParent, boundaryChild] (testId "parent")

private theorem boundaryParent_mem : boundaryParent ∈ boundaryProgram.functionInstances :=
  Array.mem_iff_getElem.mpr ⟨0, by decide, rfl⟩

private theorem boundaryChild_mem : boundaryChild ∈ boundaryProgram.functionInstances :=
  Array.mem_iff_getElem.mpr ⟨1, by decide, rfl⟩

private theorem boundaryChild_callee :
    boundaryChild ∈ calleeFunctionInstances boundaryProgram boundaryParent :=
...
theorem negative_uncontained_exit_fails_geometry : ¬ ProgramGeometry boundaryProgram := by
  intro geom
  have hpc : functionInstanceExecutionPcs boundaryProgram boundaryChild (BitVec.ofNat 64 150) :=
    Or.inl ⟨{ start := 140, size := 20 }, Array.mem_iff_getElem.mpr ⟨0, by decide, rfl⟩,
      by decide, by decide⟩
  have hexit : functionInstanceExitPred boundaryParent (BitVec.ofNat 64 150) := by
  ...
  simp [functionInstanceExitPred, FunctionInstance.isExit] at h

/-! ## 6. A function instance with no catalog dispatch cannot be discharged

`functionInstanceObligation` and `functionInstanceLocalTraceObligation` both fall to `False` when
`catalogEntryFor` returns `none`. So a function instance whose identity is not in the catalog does not
...
function instance cannot be discharged, so a program is only accepted if every function instance dispatches. -/
theorem negative_missing_dispatch_closed (p : ContractParams) :
    functionInstanceObligation p rankedProgram leaf = False := by
  unfold functionInstanceObligation; rw [uncataloged_has_no_dispatch]
...
theorem negative_missing_dispatch_local (p : ContractParams) :
    functionInstanceLocalTraceObligation p rankedProgram leaf = False := by
  unfold functionInstanceLocalTraceObligation; rw [uncataloged_has_no_dispatch]
