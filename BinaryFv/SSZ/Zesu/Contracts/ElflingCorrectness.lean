...
`sszElflingCorrectness` is not a bag of unrelated propositions. Its load-bearing content is a
...
(`sszElflingCorrectness_perFunctionInstance`), which is what the name promises. Crucially, the entry's local
...
def sourceProvenanceRecorded (program : Elfling) : Prop :=
...
cannot pass. -/
def IsCanonicalGeneratedElfling (program : Elfling) : Prop :=
...
forbids). -/
def calleeFunctionInstances (program : Elfling) (functionInstance : FunctionInstance) : Array FunctionInstance :=
...
theorem calleeFunctionInstances_subset {program : Elfling} {functionInstance callee : FunctionInstance}
...
def functionInstanceLocalObligation (p : ContractParams) (program : Elfling)
...
accepted. -/
def CallGraphRanked (program : Elfling) (rank : FunctionInstance → Nat) : Prop :=
...
theorem global_of_local {program : Elfling} {p : ContractParams} {rank : FunctionInstance → Nat}
...
def LocalToGlobal (program : Elfling) (p : ContractParams) : Prop :=
  program.entry.function = zesuDecodeRawFunctionId ∧
...
`sszElflingCorrectness_perFunctionInstance` — rather than assumed). `IsCanonicalEnvironment` pins the
...
def sszElflingCorrectness (program : Elfling) (p : ContractParams) : Prop :=
  IsCanonicalGeneratedElfling program ∧
...
/-- The per-function-instance **global** obligation is a *consequence* of `sszElflingCorrectness`, derived
...
theorem sszElflingCorrectness_perFunctionInstance {program : Elfling} {p : ContractParams}
...
def sszComplianceObligations (program : Elfling) : Prop :=
  sszElflingCorrectness program canonicalContractParams ∧ knownDivergences

/-- Coverage plus the composition entails that the specific function instance at a cataloged identity
implements its routine's correctness claim. This is the lemma that makes "`sszElflingCorrectness`
...
theorem function_instance_implements_its_contract
  ...
  have h := sszElflingCorrectness_perFunctionInstance correct functionInstance mem
