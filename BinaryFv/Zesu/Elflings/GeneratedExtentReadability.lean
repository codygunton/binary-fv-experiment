...
plus the *absorbed* excluded-function-instance ranges) and `rangesOf` for identities that name an
excluded function instance rather than a fully described function instance. Nothing checked readability there.
...
every absorbed excluded function instance sits above everything the image loads and the old check does not look.
...
instantiating at `Artifacts.programImage` never makes the kernel reduce the ELF parse. -/
...
`Artifacts.programImage`. This is the fact `bytesReadableIn` does not carry. -/
theorem executionExtentReadable_true :
    executionExtentReadableIn Artifacts.programImage generatedProgram = true := by native_decide
...
theorem generated_execution_pcs_readable :
    ∀ functionInstance ∈ generatedProgram.functionInstances, ∀ pc : BitVec 64,
      functionInstanceExecutionPcs generatedProgram functionInstance pc →
        ∃ byte, Artifacts.programImage.readByte? pc.toNat = some byte :=
...
open BinaryFv.Zesu.Entrypoints.ZesuDecodeRaw in
...
mutant below is the diff between the two checks made concrete: one absorbed excluded function instance is
...
/-- The generated program with every absorbed excluded function instance relocated above everything the image
...
/-- **The mutation is real.** At least one excluded function instance's regions genuinely moved — stated so a
...
theorem unloadedExcluded_passes_regionCheck :
    bytesReadableIn Artifacts.programImage programWithUnloadedExcluded = true := by native_decide
...
theorem unloadedExcluded_fails_extentCheck :
    executionExtentReadableIn Artifacts.programImage programWithUnloadedExcluded = false := by
  native_decide
