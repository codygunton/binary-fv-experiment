...
the source function's DWARF-resolved declaration (`Generated.generatedDeclLines`), not merely positive.
...
/-! ## 2. Every function instance's declaration line matches its source function's resolved declaration -/

/-- Whether a function instance's declaration line equals the DWARF-resolved declaration line recorded for
its source function in `generatedDeclLines`. -/
...
/-- **Every generated function instance's `declSpan` line is its source function's resolved declaration line.** Since
`generatedDeclLines` carries one line per source function (and the generator surfaces a defect if two
function instances of a source function disagree), this is the "declaration span matches the resolved source
