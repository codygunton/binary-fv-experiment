...
theorem memcpy_body_satisfies_source_post (args : CopyArgs) (fromStep : Nat) (state : State)
  ...
  have destinationMemory : DecodedValue.MemoryBytes final args.destination
      args.contents := by
    intro index inBounds
    ...
    exact copiedIndex
  have sourceMemoryFinal : DecodedValue.MemoryBytes final args.source args.contents := by
    intro index inBounds
    have argsBounds : index < args.length := by simpa [contentsLength] using inBounds
...
theorem memcpy_return_step (stepNo : Nat) (args : CopyArgs) (returnPc : BitVec 64)
  ...
  have loaded : Artifacts.programImage.fileBytesLoadedFaithfully childExit.mem := by
