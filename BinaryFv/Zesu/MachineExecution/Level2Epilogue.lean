...
/-! ## Write-set frames and the shared opening phase

Two costs lived here and both were avoidable.

`stackAgree` was proved by `cases register <;> simp_all`, splitting all forty-odd RISC-V registers
against a six-deep state term: **11.4 s of `wrapper_epilogue_to_exit`'s 13.3 s**, 8.5 s of it in the
closing `simp_all`. But `addi sp, sp, imm` writes only the bookkeeping registers and `x2`, and
`decoderPreserved` is disjoint from that, so the write-set frame settles it without looking at the
state at all.

And `wrapper_epilogue_to_exit` and `wrapper_epilogue_complete` opened with a character-identical
34-line block -- the first stack restore, the `ra` reload, and eight facts about the result. All of
it ran twice. It is proved once below, against explicit state expressions; callers keep their
`let`-bound names, since `let` is definitionally transparent. -/

/-- `decoderPreserved r` is `r ≠ x1 ∧ platformPreserved r`, so it inherits the platform's
disjointness from the bookkeeping write set. -/
...
/-- Agreement carried through the first stack restore. -/
theorem epilogue_afterFirst_agree {base state : State} (agree : Agree decoderPreserved base state)
...
/-- Agreement carried through the first stack restore and the `ra` reload. -/
theorem epilogue_afterRa_agree {base state : State} (agree : Agree decoderPreserved base state)
...
theorem epilogue_afterRa_code {state : State} (code : canonicalContractParams.env.CodeIntact state)
    (retiredFirst retiredRa stack link : BitVec 64) :
    canonicalContractParams.env.CodeIntact
      (afterRegisterWrite (wrapperAfterFirstStackRestore state retiredFirst stack)
        (BitVec.ofNat 64 0x10364) retiredRa x1 link) := by
  exact codeIntact_of_mem_eq rfl code

/-- Neither instruction touches memory. -/
theorem epilogue_afterRa_mem {state : State} (retiredFirst retiredRa stack link : BitVec 64) :
    (afterRegisterWrite (wrapperAfterFirstStackRestore state retiredFirst stack)
      (BitVec.ofNat 64 0x10364) retiredRa x1 link).mem = state.mem := rfl

/-- The stack pointer after the `+560` restore. -/
theorem epilogue_afterRa_sp {state : State} (retiredFirst retiredRa stack link : BitVec 64) :
    (afterRegisterWrite (wrapperAfterFirstStackRestore state retiredFirst stack)
...
theorem epilogue_afterRa_a0 {state : State} {result : BitVec 64}
...
theorem epilogue_afterRa_a1 {state : State} {status : BitVec 64}
    (statusValue : state.regs.get? x11 = some status)
    (retiredFirst retiredRa stack link : BitVec 64) :
    (afterRegisterWrite (wrapperAfterFirstStackRestore state retiredFirst stack)
...
theorem epilogue_afterRa_pc {state : State} (retiredFirst retiredRa stack link : BitVec 64) :
    (afterRegisterWrite (wrapperAfterFirstStackRestore state retiredFirst stack)
...
theorem wrapper_epilogue_restore_saved_registers {base state : State} {machineArgs : DecoderMachineArgs}
  ...
  have codeS0 : canonicalContractParams.env.CodeIntact afterS0 := by
    exact codeIntact_of_mem_eq (afterRegisterWrite_mem _ _ _ _ _) code
  ...
  have codeS1 : canonicalContractParams.env.CodeIntact afterS1 := by
    exact codeIntact_of_mem_eq (afterRegisterWrite_mem _ _ _ _ _) codeS0
  ...
  · exact codeIntact_of_mem_eq (afterRegisterWrite_mem _ _ _ _ _) codeS1
  · exact agreeS2
...
theorem wrapper_epilogue_to_exit {base state : State} {machineArgs : DecoderMachineArgs}
  ...
  · exact codeIntact_of_mem_eq rfl saved.code
