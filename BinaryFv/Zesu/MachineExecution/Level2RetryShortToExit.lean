...
theorem retry_short_rejection_to_exit
  ...
  obtain ⟨used, childAfter, retired, edge⟩ :=
    retry_short_rejection_edge fromStep args before pre phase short link s0 s1 s2 saved
  have handoffMachine : DecoderMachinePre
      (functionInstanceExecutionPcs generatedProgram functionInstance_raw_decoder_root_zesu_decode_raw)
      args.machineArgs (decodeInlineRetryShortBranchAfter childAfter retired) :=
    DecoderMachinePre.mono edge.handoffAgree edge.handoffRetired machine
  have handoffStatus : (decodeInlineRetryShortBranchAfter childAfter retired).regs.get? x11 =
    ...
    simpa [decodeInlineRetryShortBranchAfter, tryStepControlFlowAfterRetired,
      ...
      tryStepControlFlowAfterIncrement, Std.ExtDHashMap.get?_insert] using edge.status
  obtain ⟨afterTail, afterStore, after, suffix⟩ := retry_rejection_to_exit machineEntry handoffMachine
    ...
    handoffStatus link s0 s1 s2 edge.handoffFrame
  ...
  refine ⟨used, childAfter, decodeInlineRetryShortBranchAfter childAfter retired, afterTail, afterStore, after, ?_⟩
  exact ⟨edge, suffix,
    ...
    suffix.savedFrame, suffix.memory, suffix.code, suffix.retired⟩
