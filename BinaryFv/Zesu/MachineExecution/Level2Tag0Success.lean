...
structure Tag0StoredResultCopyPre (args : ZesuDecodeRawArgs)
    (value : BinaryFv.Specs.SSZ.StatelessInput) (stackBase : Nat) (entry state : State)
  ...
  sourceBytes : DecodedValue.MemoryBytes state (stackBase + 32) contents
  contentsSize : contents.size = 832
  sourceRepresentation : ∃ cursorBefore cursorAfter,
    StatelessInputRepInHeapInterval state args.inputBase args.bytes (stackBase + 32) value
...
structure Tag0StoredResultCopyPhase (args : ZesuDecodeRawArgs)
    (value : BinaryFv.Specs.SSZ.StatelessInput) (stackBase : Nat) (entry state : State)
  ...
  destinationBytes : DecodedValue.MemoryBytes resumed 0x4215030 contents
  contentsSize : contents.size = 832
  copyBound : used ≤ 64 + 8 * 832
  destinationRepresentation : ∃ cursorBefore cursorAfter,
    StatelessInputRepInHeapInterval resumed args.inputBase args.bytes 0x4215030 value
      cursorBefore cursorAfter ∧ cursorAfter ≤ Elflings.canonicalHeapLimit
  ...
  inputMemory : DecodedValue.MemoryBytes resumed args.inputBase args.bytes
  platform : Agree decoderPreserved state resumed
  globalsFrame : DecoderGlobalsBoundaryFrame state resumed
...
theorem tag0_stored_result_setup
    ...
    ∃ r0 r1 r2 r3 callState,
      ...
      DecodedValue.MemoryBytes callState (stackBase + 32) contents := by
...
theorem tag0_stored_result_memcpy_machine_pre
    {args : ZesuDecodeRawArgs} {value : BinaryFv.Specs.SSZ.StatelessInput} {stackBase : Nat}
    {entry state childEntry : State}
    (contents : ByteArray) (pre : Tag0StoredResultCopyPre args value stackBase entry state
...
theorem tag0_stored_result_copy_phase
    ...
    (pre : Tag0StoredResultCopyPre args value stackBase entry state contents link savedS0 savedS1 savedS2)
    (fromStep : Nat) :
    ∃ used callState resumed, Tag0StoredResultCopyPhase args value stackBase entry state contents link savedS0
  ...
  have childCode : canonicalContractParams.env.CodeIntact childEntry := by
    ...
    exact pre.code
  have childSourceMemory : DecodedValue.MemoryBytes childEntry (stackBase + 32) contents := by
    intro index bound
  ...
  rcases sourcePost with ⟨exitCode, -, -, copyFrame, copiedSource, destinationMemory⟩
  ...
  have resumedInputMemory : DecodedValue.MemoryBytes resumed args.inputBase args.bytes := by
    apply pre.inputMemory.of_mem_eq
    intro index indexBound
    rw [show resumed.mem = childExit.mem by rfl]
    rw [copyFrame (args.inputBase + index) (by
      rcases pre.machineEntry.inputAvoidsDecoderGlobals with inputBefore | globalsBefore
      · left
        have destinationAboveGlobals : Elflings.GeneratedDecoderGlobals.bssBase ≤ 0x4215030 := by
          native_decide
        simpa [tag0StoredResultCopyArgs] using
          show args.inputBase + index < 0x4215030 by omega
      · right
        have destinationWithinGlobals : 0x4215030 + 832 ≤
            Elflings.GeneratedDecoderGlobals.bssBase + Elflings.GeneratedDecoderGlobals.bssSize := by
          native_decide
        simpa [tag0StoredResultCopyArgs] using
          show 0x4215030 + 832 ≤ args.inputBase + index by omega)]
    rw [show childEntry.mem = state.mem by simpa [childEntry] using callMemory]
  have globalsFrame : DecoderGlobalsBoundaryFrame state resumed := by
    have childEntryMemory : childEntry.mem = state.mem := by
      simpa [childEntry, afterRegisterWrite_mem] using callMemory
    ...
    have storedTagAddress : Elflings.canonicalDecoderGlobalsLayout.storedResult +
      ...
      native_decide
    have attemptedOutside : 0x4215020 < (tag0StoredResultCopyArgs stackBase contents).destination := by
      change 0x4215020 < 0x4215030
      native_decide
    have storedTagOutside : (tag0StoredResultCopyArgs stackBase contents).destination +
        (tag0StoredResultCopyArgs stackBase contents).length ≤ 0x4215370 := by
      change 0x4215030 + 832 ≤ 0x4215370
      native_decide
    constructor
    · rw [show resumed.mem = childExit.mem by rfl]
      rw [attemptedAddress]
      rw [copyFrame 0x4215020 (Or.inl attemptedOutside)]
      exact congrArg (fun mem => mem.get? 0x4215020) childEntryMemory
    · rw [show resumed.mem = childExit.mem by rfl]
      rw [storedTagAddress]
      rw [copyFrame 0x4215370 (Or.inr storedTagOutside)]
      exact congrArg (fun mem => mem.get? 0x4215370) childEntryMemory
  obtain ⟨cursorBefore, cursorAfter, sourceRepresentation, cursorBound⟩ :=
    pre.sourceRepresentation
  have childEntryRepresentation : StatelessInputRepInHeapInterval childEntry args.inputBase args.bytes
      (stackBase + 32) value cursorBefore cursorAfter :=
    sourceRepresentation.of_mem_eq (by simpa [childEntry] using callMemory)
  have childExitSourceRepresentation : StatelessInputRepInHeapInterval childExit args.inputBase
      args.bytes (stackBase + 32) value cursorBefore cursorAfter :=
    childEntryRepresentation.survives_copy (tag0StoredResultCopyArgs stackBase contents) copyFrame
      (by
        intro address root
        right
        have separated := Entrypoints.ZesuDecodeRaw.wrapper_stack_after_stored_result pre.machineEntry
        dsimp [range, tag0StoredResultCopyArgs] at root ⊢
        omega)
      (by
        intro address interval
        left
        dsimp [Contracts.interval, tag0StoredResultCopyArgs] at interval ⊢
        have heapBeforeGlobals : Elflings.canonicalHeapLimit ≤ 0x4215030 := by native_decide
        omega)
  have childExitDestinationRepresentation : StatelessInputRepInHeapInterval childExit args.inputBase
      args.bytes 0x4215030 value cursorBefore cursorAfter :=
    childExitSourceRepresentation.rebase_root (by native_decide) (by
      intro index indexBound
      have destinationMemory' : DecodedValue.MemoryBytes childExit 0x4215030 contents := by
        simpa [tag0StoredResultCopyArgs, meaningCopy] using destinationMemory
      exact (destinationMemory' index (by simpa [pre.contentsSize] using indexBound)).trans
        (copiedSource index (by simpa [tag0StoredResultCopyArgs, pre.contentsSize] using indexBound)).symm)
  have resumedDestinationRepresentation : StatelessInputRepInHeapInterval resumed args.inputBase
      args.bytes 0x4215030 value cursorBefore cursorAfter :=
    childExitDestinationRepresentation.of_mem_eq (by
      simp [resumed, memcpyReturnAfter, tryStepControlFlowAfterRetired,
        tryStepControlFlowAfterTick, controlFlowJumpState, tryStepControlFlowAfterIncrement,
        coreControlFlowNextState])
  refine ⟨used,
  ...
  refine ⟨pre.machineEntry, setupPrefix, ⟨transfer⟩, scopedPrefix,
    ?_, by simpa [resumed] using resumePc, resumedFrame, ?_, pre.contentsSize, ?_,
      ⟨cursorBefore, cursorAfter, resumedDestinationRepresentation,
        cursorBound⟩, ?_, ?_, ?_, ?_,
      resumedMachine, resumedInputMemory, resumedAgree, globalsFrame⟩
  ...
  · simpa [compiledMemcpyContract, Contracts.contractMemcpy, tag0StoredResultCopyArgs] using bound
