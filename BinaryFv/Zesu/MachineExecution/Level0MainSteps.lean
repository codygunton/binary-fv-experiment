...
theorem writeSuccessMemcpyCallBaseStep (stepNo : Nat) (state : State)
  ...
  exact configuredAuipcStep stepNo state 0x14d78 0xffffb 0x97 0xb0 0xff 0xff
    configured atPc loaded (decode := by
...
theorem configuredJalrCallStep (stepNo : Nat) (state : State) (pc : Nat)
    ...
    (targetEq : Sail.BitVec.update (callBase + sign_extend (m := 64) imm) 0 0#1 = target := by
      native_decide)
    (returnEq : Sail.BitVec.addInt (BitVec.ofNat 64 pc) 4 = returnPc := by native_decide)
    (targetBit1 : Sail.BitVec.access (callBase + sign_extend (m := 64) imm) 1 = 0#1 := by
      native_decide) :
    ∃ retired, Runs (try_step stepNo false) state
      (tryStepControlFlowAfterRetired
...
theorem main_read_input_call_step (stepNo : Nat) (state : State)
  ...
  exact configuredJalrCallStep stepNo state 0x14cc8 0xfcc4 0x47c 0x10140 0x14ccc
    0xe7 0x80 0xc0 0x47 configured atPc callBase loaded
...
theorem main_decode_result_address_step (stepNo : Nat) (state : State)
  ...
  let premise := coreControlFlowNextState (tryStepControlFlowAfterIncrement state) 0x14cec
  have execute : Runs (execute (.ITYPE (0x20, stackPointer, .Regidx 10#5, .ADDI)))
      premise { premise with regs := premise.regs.insert x10 (iTypeResult .ADDI 0x20 source.value) }
      (.Retire_Success ()) := by
    change Runs (execute_ITYPE 0x20 stackPointer (.Regidx 10#5) .ADDI) _ _ _
    exact execute_ITYPE_run _ _ 0x20 stackPointer (.Regidx 10#5) .ADDI source.value
      source.read (main_wX_bits_run_x10 _ _)
  simpa [afterRegisterWrite] using
    MachineExecution.configuredRegisterWriteStep stepNo 0x14cec state x10
      (iTypeResult .ADDI 0x20 source.value)
      (.ITYPE (0x20, stackPointer, .Regidx 10#5, .ADDI)) 0x13 0x05 0x01 0x02
      configured atPc loaded (by configured_decode configured) execute (base := by rfl)
...
theorem main_decode_allocator_address_step (stepNo : Nat) (state : State)
  ...
  let premise := coreControlFlowNextState (tryStepControlFlowAfterIncrement state) 0x14cf0
  have execute : Runs (execute (.ITYPE (0x10, stackPointer, .Regidx 11#5, .ADDI)))
      premise { premise with regs := premise.regs.insert x11 (iTypeResult .ADDI 0x10 source.value) }
      (.Retire_Success ()) := by
    change Runs (execute_ITYPE 0x10 stackPointer (.Regidx 11#5) .ADDI) _ _ _
    exact execute_ITYPE_run _ _ 0x10 stackPointer (.Regidx 11#5) .ADDI source.value
      source.read (main_wX_bits_run_x11 _ _)
  simpa [afterRegisterWrite] using
    MachineExecution.configuredRegisterWriteStep stepNo 0x14cf0 state x11
      (iTypeResult .ADDI 0x10 source.value)
      (.ITYPE (0x10, stackPointer, .Regidx 11#5, .ADDI)) 0x93 0x05 0x01 0x01
      configured atPc loaded (by configured_decode configured) execute (base := by rfl)
...
theorem main_decode_call_step (stepNo : Nat) (state : State)
  ...
  exact configuredJalrCallStep stepNo state 0x14cf8 0x11cf4 0x474 0x12168 0x14cfc
    0xe7 0x80 0x40 0x47 configured atPc callBase loaded
    (decode := by configured_decode configured)
...
theorem main_success_result_address_step (stepNo : Nat) (state : State)
  ...
  let premise := coreControlFlowNextState (tryStepControlFlowAfterIncrement state) 0x14d04
  have execute : Runs (execute (.ITYPE (0x20, stackPointer, .Regidx 10#5, .ADDI)))
      premise { premise with regs := premise.regs.insert x10 (iTypeResult .ADDI 0x20 source.value) }
      (.Retire_Success ()) := by
    change Runs (execute_ITYPE 0x20 stackPointer (.Regidx 10#5) .ADDI) _ _ _
    exact execute_ITYPE_run _ _ 0x20 stackPointer (.Regidx 10#5) .ADDI source.value
      source.read (main_wX_bits_run_x10 _ _)
  simpa [afterRegisterWrite] using
    MachineExecution.configuredRegisterWriteStep stepNo 0x14d04 state x10
      (iTypeResult .ADDI 0x20 source.value)
      (.ITYPE (0x20, stackPointer, .Regidx 10#5, .ADDI)) 0x13 0x05 0x01 0x02
      configured atPc loaded (by configured_decode configured) execute (base := by rfl)
...
theorem main_write_success_call_base_step (stepNo : Nat) (state : State)
  ...
  exact configuredAuipcStep stepNo state 0x14d08 0 0x97 0x00 0x00 0x00 configured atPc loaded
    (decode := by
    obtain ⟨seccfgBits, _, _, privilegeAfter, seccfgAfter⟩ :=
      configured.decodeContext
    unfold Runs
...
theorem main_write_success_call_step (stepNo : Nat) (state : State)
  ...
  exact configuredJalrCallStep stepNo state 0x14d0c 0x14d08 0x028 0x14d30 0x14d10
    0xe7 0x80 0x80 0x02 configured atPc callBase loaded
    (decode := by configured_decode configured)
...
theorem main_success_exit_call_base_step (stepNo : Nat) (state : State)
  ...
  exact configuredAuipcStep stepNo state 0x14d14 0xffffb 0x97 0xb0 0xff 0xff configured atPc loaded
    (decode := main_auipc_neg5_decode state configured)
...
theorem main_write_failure_call_base_step (stepNo : Nat) (state : State)
  ...
  exact configuredAuipcStep stepNo state 0x14d1c 1 0x97 0x10 0x00 0x00 configured atPc loaded
    (decode := main_auipc_plus1_decode state configured)
...
theorem main_failure_exit_call_base_step (stepNo : Nat) (state : State)
  ...
  exact configuredAuipcStep stepNo state 0x14d28 0xffffb 0x97 0xb0 0xff 0xff configured atPc loaded
    (decode := main_auipc_neg5_decode state configured)
...
theorem main_success_exit_call_step (stepNo : Nat) (state : State)
  ...
  exact configuredJalrCallStep stepNo state 0x14d18 0xfd14 0x4b0 0x101c4 0x14d1c
    0xe7 0x80 0x00 0x4b configured atPc baseRead loaded
    (decode := main_jalr_decode state configured 0xe7 0x80 0x00 0x4b 0x4b0
...
theorem main_write_failure_call_step (stepNo : Nat) (state : State)
  ...
  exact configuredJalrCallStep stepNo state 0x14d20 0x15d1c 0x4a4 0x161c0 0x14d24
    0xe7 0x80 0x40 0x4a configured atPc baseRead loaded
    (decode := main_jalr_decode state configured 0xe7 0x80 0x40 0x4a 0x4a4
...
theorem main_failure_exit_call_step (stepNo : Nat) (state : State)
    ...
    ∃ retired, Runs (try_step stepNo false) state
      (tryStepControlFlowAfterRetired
        ...
        0x101c4 retired) false := by
  exact configuredJalrCallStep stepNo state 0x14d2c 0xfd28 0x49c 0x101c4 0x14d30
    0xe7 0x80 0xc0 0x49 configured atPc baseRead loaded
    (decode := main_jalr_decode state configured 0xe7 0x80 0xc0 0x49 0x49c
