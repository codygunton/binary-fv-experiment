...
structure Tag0PostMemcpyPre (base state : State) (machineArgs : DecoderMachineArgs)
    (value : BinaryFv.Specs.SSZ.StatelessInput) where
  ...
  contents : ByteArray
  payload : DecodedValue.MemoryBytes state 0x4215030 contents
  payloadLength : contents.size = 832
  representation : ∃ cursorBefore cursorAfter,
    StatelessInputRepInHeapInterval state machineArgs.inputBase machineArgs.bytes 0x4215030 value
  ...
  inputMemory : DecodedValue.MemoryBytes state machineArgs.inputBase machineArgs.bytes
  inputAvoidsDecoderGlobals :
    machineArgs.inputBase + machineArgs.bytes.size ≤ Elflings.GeneratedDecoderGlobals.bssBase ∨
      Elflings.GeneratedDecoderGlobals.bssBase + Elflings.GeneratedDecoderGlobals.bssSize ≤
        machineArgs.inputBase

/-- Execute `addi a1, x0, 1` at `0x10350`. -/
theorem tag0_postcopy_status_register_step {machineArgs : DecoderMachineArgs}
    {value : BinaryFv.Specs.SSZ.StatelessInput} {base state : State}
    (pre : Tag0PostMemcpyPre base state machineArgs value) (stepNo : Nat) :
...
theorem tag0PostcopyStatusStoreAfter_preserves_payload {state : State} (retired : BitVec 64)
    (contents : ByteArray) (payload : DecodedValue.MemoryBytes state 0x4215030 contents)
    (length : contents.size = 832) :
    DecodedValue.MemoryBytes (tag0PostcopyStatusStoreAfter state retired) 0x4215030 contents := by
...
/-- The tag-zero discriminant store is inside the validated decoder `.bss` interval, so it leaves
the separately required borrowed input interval unchanged. -/
theorem tag0PostcopyStatusStoreAfter_preserves_input {state : State} (retired : BitVec 64)
    {inputBase : Nat} {bytes : ByteArray} (input : DecodedValue.MemoryBytes state inputBase bytes)
    (separated : inputBase + bytes.size ≤ Elflings.GeneratedDecoderGlobals.bssBase ∨
      Elflings.GeneratedDecoderGlobals.bssBase + Elflings.GeneratedDecoderGlobals.bssSize ≤ inputBase) :
    DecodedValue.MemoryBytes (tag0PostcopyStatusStoreAfter state retired) inputBase bytes := by
  apply input.of_mem_eq
  intro index indexBound
  have outside : ∀ storeIndex : Fin 1, 0x4215370 + storeIndex.val ≠ inputBase + index := by
    intro storeIndex equal
    rcases separated with inputBefore | globalsBefore
    · have storeInGlobals : Elflings.GeneratedDecoderGlobals.bssBase ≤ 0x4215370 := by
        native_decide
      omega
    · have storeEndInGlobals : 0x4215370 + 1 ≤ Elflings.GeneratedDecoderGlobals.bssBase +
          Elflings.GeneratedDecoderGlobals.bssSize := by native_decide
      omega
  have preserved := afterWriteBytes_mem_get?_of_outside
    (coreControlFlowNextState (tryStepControlFlowAfterIncrement state) (BitVec.ofNat 64 0x10354))
    0x4215370 (BitVec.ofNat 64 1) (inputBase + index) outside
  simpa [tag0PostcopyStatusStoreAfter, tryStepControlFlowAfterRetired, tryStepControlFlowAfterTick,
    coreControlFlowNextState, tryStepControlFlowAfterIncrement] using preserved
...
theorem tag0_postcopy_result_register_step {machineArgs : DecoderMachineArgs} {base state : State}
  ...
  have decode : Runs (ext_decode (0x00100513 : BitVec 32))
      (tryStepControlFlowAfterIncrement state) (tryStepControlFlowAfterIncrement state)
      (.ITYPE (0x001#12, .Regidx 0#5, .Regidx 10#5, .ADDI)) := by decode_run
  let executeState := coreControlFlowNextState (tryStepControlFlowAfterIncrement state)
  ...
  exact generatedRegisterWriteStep machine agree retiredPresent
    (hasExactErePrefix_programImage_of_codeIntact code) stepNo 0x10358 0x00100513 atPc decode execute
...
structure Tag0PostcopyResult (base before after : State) (fromStep : Nat)
    (contents : ByteArray) (inputBase : Nat) (input : ByteArray)
    (value : BinaryFv.Specs.SSZ.StatelessInput) (stack link savedS0 savedS1 savedS2 : BitVec 64) : Prop where
  ...
  payloadLength : contents.size = 832
  payload : DecodedValue.MemoryBytes after 0x4215030 contents
  representation : ∃ cursorBefore cursorAfter,
    StatelessInputRepInHeapInterval after inputBase input 0x4215030 value cursorBefore cursorAfter ∧
      cursorAfter ≤ Elflings.canonicalHeapLimit
  ...
  inputMemory : DecodedValue.MemoryBytes after inputBase input
...
def Tag0PostcopyResult.terminal
    (result : Tag0PostcopyResult base before after fromStep contents inputBase input value stack link
...
theorem tag0_postcopy_complete {machineArgs : DecoderMachineArgs}
    {value : BinaryFv.Specs.SSZ.StatelessInput} {base before : State}
    (pre : Tag0PostMemcpyPre base before machineArgs value) (fromStep : Nat) :
    ∃ after, Tag0PostcopyResult base before after fromStep pre.contents machineArgs.inputBase
      machineArgs.bytes value pre.stack pre.link
  ...
  have payload1 : DecodedValue.MemoryBytes s1 0x4215030 pre.contents := by
  ...
  have input1 : DecodedValue.MemoryBytes s1 machineArgs.inputBase machineArgs.bytes := by
    apply pre.inputMemory.of_mem_eq
    intro index indexBound
    simp [s1, afterRegisterWrite_mem]
  have input2 : DecodedValue.MemoryBytes s2 machineArgs.inputBase machineArgs.bytes :=
    tag0PostcopyStatusStoreAfter_preserves_input r2 input1 pre.inputAvoidsDecoderGlobals
  have inputAfter : DecodedValue.MemoryBytes after machineArgs.inputBase machineArgs.bytes := by
    apply input2.of_mem_eq
    intro index indexBound
  ...
  have attemptedFrame : after.mem.get? Elflings.canonicalDecoderGlobalsLayout.attempted =
    ...
    have attemptedAddress : Elflings.canonicalDecoderGlobalsLayout.attempted = 0x4215020 := by
      native_decide
    rw [show after.mem = s2.mem by exact afterRegisterWrite_mem _ _ _ _ _]
    rw [show s2.mem.get? Elflings.canonicalDecoderGlobalsLayout.attempted =
      s1.mem.get? Elflings.canonicalDecoderGlobalsLayout.attempted by
      simpa [s2, tag0PostcopyStatusStoreAfter] using afterWriteBytes_mem_get?_of_outside
        (coreControlFlowNextState (tryStepControlFlowAfterIncrement s1) (BitVec.ofNat 64 0x10354))
        0x4215370 (BitVec.ofNat 64 1) Elflings.canonicalDecoderGlobalsLayout.attempted (by
          intro index equal
          rw [attemptedAddress] at equal
          omega)]
    exact congrArg (fun mem => mem.get? Elflings.canonicalDecoderGlobalsLayout.attempted)
  ...
  have storedTag : DecodedValue.OptionTagRep after
    ...
    rw [storedTagAddress]
    change after.mem.get? 0x4215370 = some (1#8)
    rw [show after.mem = s2.mem by exact afterRegisterWrite_mem _ _ _ _ _]
    change (tag0PostcopyStatusStoreAfter s1 r2).mem.get? 0x4215370 = some (1#8)
    change (s1.mem.insert 0x4215370 (1#8)).get? 0x4215370 = some (1#8)
    simp [Std.ExtHashMap.get?_eq_getElem?]
  obtain ⟨cursorBefore, cursorAfter, representation, cursorBound⟩ :=
    pre.representation
  obtain ⟨bases, heapWithin, transportRepresentation⟩ := representation.transport
  have representationAfter : StatelessInputRepInHeapInterval after machineArgs.inputBase
      machineArgs.bytes 0x4215030 value cursorBefore cursorAfter := transportRepresentation (by
    intro address footprint
    have beforeTag : address < 0x4215370 := by
      rcases footprint with root | heap
      · dsimp [range] at root
        omega
      · have within := heapWithin address heap
        have heapBeforeTag : Elflings.canonicalHeapLimit ≤ 0x4215370 := by native_decide
        omega
    change before.mem.get? address = (before.mem.insert 0x4215370 (1#8)).get? address
    simp only [Std.ExtHashMap.get?_eq_getElem?, Std.ExtHashMap.getElem?_insert, beq_iff_eq]
    have different : 0x4215370 ≠ address := by omega
    rw [if_neg different])
  ...
  have prefix3 : WrapperPrefix (fromStep + 2) 1 s2 after :=
    ConfinedPrefix.ownStep' pc2 (by simpa [after] using h3)
  have confined : WrapperPrefix fromStep 3 before after := by
    confined_steps [prefix1, prefix2, prefix3]
  refine ⟨after, ?_⟩
  refine ⟨?_, confined, ?_, ?_, ?_, pre.payloadLength, ?_,
    ⟨cursorBefore, cursorAfter, representationAfter, cursorBound⟩, ?_,
    pre.payloadBeforeStack,
    ?_, ?_, ?_, ?_, ?_, ?_, attemptedFrame, storedTag⟩
...
theorem tag0_postcopy_through_epilogue {machineArgs : DecoderMachineArgs}
    {value : BinaryFv.Specs.SSZ.StatelessInput} {base before : State}
    (pre : Tag0PostMemcpyPre base before machineArgs value) (fromStep : Nat) (restoredStack : BitVec 64)
    ...
    ∃ routeAfter afterStore after,
      Tag0PostcopyResult base before routeAfter fromStep pre.contents machineArgs.inputBase
        machineArgs.bytes value pre.stack pre.link
