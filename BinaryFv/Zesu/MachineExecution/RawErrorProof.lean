...
the runner-facing retirement of that `ret` is already handled by `SentinelAssembly.lean`.
-/
...
theorem raw_error_load_step (fromStep : Nat) (state : State) (status : DecodeStatus)
  ...
  have firstCode : Artifacts.programImage.fileBytesMatchMemory afterAuipc.mem := by
