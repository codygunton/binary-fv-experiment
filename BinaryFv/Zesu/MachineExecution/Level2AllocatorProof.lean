...
theorem wrapperAfterAllocatorTag_code (state : State) (retired target data : BitVec 64)
  ...
  have executeCode : Artifacts.programImage.fileBytesLoadedFaithfully executeState.mem := by
    simpa [executeState, coreControlFlowNextState, tryStepControlFlowAfterIncrement] using code
  have stored := fileBytesLoadedFaithfully_afterWriteBytes (width := 1) Artifacts.programImage
...
private theorem allocatorInstructionStepPlatform {instructionPcs : BitVec 64 → Prop}
    ...
    (code : Artifacts.programImage.fileBytesLoadedFaithfully state.mem) (pc : Nat)
...
theorem allocatorAfterContextStore_code (state : State)
  ...
  have executeCode : Artifacts.programImage.fileBytesLoadedFaithfully executeState.mem := by
    simpa [executeState, coreControlFlowNextState, tryStepControlFlowAfterIncrement] using code
  have stored := fileBytesLoadedFaithfully_afterWriteBytes Artifacts.programImage executeState
...
inductive AllocatorSegmentExecution : Nat → Nat → State → State → Prop where
  /-- The first allocator segment has no owned steps and starts on its generated outgoing exit. -/
  | dataPointer {fromStep : Nat} {state : State}
      (trace : FunctionTrace
        ...
        (functionInstanceExitPred
          functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41)
        fromStep 0 state state) :
      AllocatorSegmentExecution fromStep 0 state state
  /-- The second allocator segment carries its three generated body steps as a confined trace
  ending at the generated outgoing instruction `0x10304`. -/
  | functionAndContext {fromStep : Nat} {entry atOutgoingEdge : State}
      (trace : FunctionTrace
        (functionInstanceExecutionPcs generatedProgram
          functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41)
        (functionInstanceExitPred
          functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41)
        fromStep 3 entry atOutgoingEdge) :
...
theorem AllocatorSegmentExecution.three_trace {fromStep : Nat} {entry exit : State}
  ...
  | functionAndContext trace => exact trace.toTrace

theorem AllocatorSegmentExecution.toFunctionTrace {fromStep used : Nat} {entry exit : State}
    (execution : AllocatorSegmentExecution fromStep used entry exit) :
    FunctionTrace
      (functionInstanceExecutionPcs generatedProgram
        functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41)
      (functionInstanceExitPred
        functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41)
      fromStep used entry exit := by
  cases execution with
  | dataPointer trace => exact trace
  | functionAndContext trace => exact trace
...
theorem allocatorChildSummary_functionAndContext {fromStep : Nat}
  ...
  exact ⟨rfl, .functionAndContext trace⟩
...
structure AllocatorSecondSegmentPreconditions (entry : State)
  ...
  code : Artifacts.programImage.fileBytesLoadedFaithfully entry.mem
...
theorem allocator_data_pointer_inlineTransfer (fromStep : Nat) (state : State)
  ...
  have body : allocatorChildSummary
      functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41Id
      fromStep 0 state state :=
    allocatorChildSummary_dataPointer (.exitAt fromStep state (BitVec.ofNat 64 0x102f0) atPc (by
...
theorem allocator_functionAndContext_inlineTransfer (fromStep : Nat)
    ...
    (atPage : afterPage.regs.get? PC = some (BitVec.ofNat 64 0x102fc))
    (addressStep : Runs (try_step (fromStep + 1) false) afterPage afterAddress false)
    (atAddress : afterAddress.regs.get? PC = some (BitVec.ofNat 64 0x10300))
  ...
  have bodyTrace : FunctionTrace
      (functionInstanceExecutionPcs generatedProgram
        functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41)
      (functionInstanceExitPred
        functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41)
      fromStep 3 entry atOutgoingEdge := by
    apply FunctionTrace.step fromStep 2 (BitVec.ofNat 64 0x102f8) entry afterPage atOutgoingEdge
      atEntry (regionPc _) (notExitPc _) pageStep
    apply FunctionTrace.step (fromStep + 1) 1 (BitVec.ofNat 64 0x102fc) afterPage afterAddress
      atOutgoingEdge atPage (regionPc _) (notExitPc _) addressStep
    apply FunctionTrace.step (fromStep + 2) 0 (BitVec.ofNat 64 0x10300) afterAddress atOutgoingEdge
      atOutgoingEdge atAddress (regionPc _) (notExitPc _) contextStep
    exact .exitAt (fromStep + 3) atOutgoingEdge (BitVec.ofNat 64 0x10304) atOutgoingPc (by
      simp [functionInstanceExitPred, BinaryFv.Binary.Elfling.FunctionInstance.isExit,
        functionInstance_raw_decoder_root_allocator_in_raw_decoder_root_zesu_decode_raw_at_112_41])
  refine ⟨
    { valid := allocatorInlineBoundary_valid
      ...
      body := allocatorChildSummary_functionAndContext bodyTrace
...
theorem allocator_second_segment_proved (fromStep : Nat) (entry : State)
  ...
  have pageRetiredPresent : RetiredCounterPresent afterPage :=
    afterRegisterWrite_retired_present entry (BitVec.ofNat 64 0x102f8) pageRetired x10
      (BitVec.ofNat 64 0x142f8)
  have pageCode : Artifacts.programImage.fileBytesLoadedFaithfully afterPage.mem := by
  ...
  have addressRetiredPresent : RetiredCounterPresent afterAddress :=
    afterRegisterWrite_retired_present afterPage (BitVec.ofNat 64 0x102fc) addressRetired x10
      (BitVec.ofNat 64 0x13f70)
  have addressCode : Artifacts.programImage.fileBytesLoadedFaithfully afterAddress.mem := by
  ...
  have contextRetiredPresent := allocatorAfterContextStore_retired afterAddress contextRetired
    stackBase context
  have contextCode : Artifacts.programImage.fileBytesLoadedFaithfully atOutgoingEdge.mem := by
  ...
  simpa [afterPage, afterAddress, atOutgoingEdge, afterTransfer] using
    allocator_functionAndContext_inlineTransfer fromStep entry afterPage afterAddress
      atOutgoingEdge afterTransfer pre.atEntry pageStep pagePc addressStep addressPc contextStep outgoingPc
