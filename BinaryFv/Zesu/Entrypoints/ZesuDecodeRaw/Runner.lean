...
def runnerSymbols : Option RunnerSymbols := do
  let decode ← Artifacts.zesuDecodeRaw.toOption
  let result ← Artifacts.zesuRawResult.toOption
  let error ← Artifacts.zesuRawError.toOption
...
transfer closure, which pulls in `ownedRanges` (own regions plus *absorbed* excluded-function instance ranges)
...
theorem loaded_ne_sentinelWord (pc : BitVec 64)
    (loaded : ∃ byte, Artifacts.programImage.readByte? pc.toNat = some byte) :
    pc ≠ sentinelWord :=
...
def runZesuDecodeRaw (symbols : RunnerSymbols) (input : ByteArray) :
  ...
  pure (classifyWrapperRun observeDecodedValue storedResultDiscriminantAddr
    Elflings.canonicalResultBuffer accessors.1 accessors.2 outcome afterCall)
...
theorem executeChecked_rejects_wrong_artifact {binary : RiscvSpec.ValidatedElf} (input : ByteArray)
    (h : binary.bytes ≠ Artifacts.bytes) :
...
theorem runZesuDecodeRaw_of_trace (symbols : RunnerSymbols) (input : ByteArray) {count : Nat}
    ...
    Runs (runZesuDecodeRaw symbols input) initialState after
      (classifyWrapperRun observeDecodedValue storedResultDiscriminantAddr
        Elflings.canonicalResultBuffer accessors.1 accessors.2 (.reached count) final) :=
  Runs.bind hbuild
...
theorem executeDecode_of_trace {symbols : RunnerSymbols} (input : ByteArray) {count : Nat}
    ...
    executeDecode input =
      classifyWrapperRun observeDecodedValue storedResultDiscriminantAddr
        Elflings.canonicalResultBuffer accessors.1 accessors.2 (.reached count) final := by
...
These are what `BinaryFv/Zesu/Root.lean` consumes. Each takes a complete description of one machine
...
theorem executeDecode_rejected_of_run (input : ByteArray) {entry final after : State}
  ...
  exact classifyWrapperRun_rejected observeDecodedValue storedResultDiscriminantAddr
    Elflings.canonicalResultBuffer count status _ _ final hcode rfl hstatus rfl htag
...
theorem runZesuDecodeRaw_classifies (symbols : RunnerSymbols) (input : ByteArray) {before after : State}
    ...
    ∃ outcome final rawResult rawError,
      result = classifyWrapperRun observeDecodedValue storedResultDiscriminantAddr
        Elflings.canonicalResultBuffer rawResult rawError outcome final := by
...
theorem executeDecode_accepted_forces_checks {input : ByteArray} {value : BinaryFv.Specs.SSZ.StatelessInput}
    ...
    ∃ (final : State) (steps : Nat) (rawResult rawError : AccessorOutcome),
      ...
      rawResult = AccessorOutcome.returned Elflings.canonicalResultBuffer ∧
