...
def heapCeiling : Nat :=
  (Elflings.GeneratedDecoderGlobals.runtimeGlobals.foldl
...
theorem segments_below_ceiling :
    Artifacts.programImage.segments.toList.all
...
theorem arena_disjoint_from_globals :
    heapCeiling ≤ Elflings.GeneratedDecoderGlobals.bssBase := by decide
...
theorem loaded_disjoint_from_runner (address : Nat)
    (loaded : ∃ byte, Artifacts.programImage.readByte? address = some byte) :
