...
compares its bytes to `Artifacts.bytes`, the one Nix-built image the whole proof is about. A
...
def artifactIsCanonical (binary : RiscvSpec.ValidatedElf) : Bool :=
  decide (binary.bytes = Artifacts.bytes)
...
theorem canonical_parses_as_artifact {binary : RiscvSpec.ValidatedElf}
    ...
    BinaryFv.RiscV.Elf64.parse binary.bytes = .ok Artifacts.elf := by
  have hbytes : binary.bytes = Artifacts.bytes := by
    unfold artifactIsCanonical at h; exact of_decide_eq_true h
  rw [hbytes]; exact Artifacts.parsed_ok

/-- A canonical ELF's parsed image is the canonical program image the contracts pin. So once the gate
passes, `Artifacts.programImage`-based framing (the runner layout's `loaded_disjoint_from_runner`, the
...
theorem canonical_image_is_programImage {binary : RiscvSpec.ValidatedElf}
    (h : artifactIsCanonical binary = true) :
    binary.elf.programImage = Artifacts.programImage := by
