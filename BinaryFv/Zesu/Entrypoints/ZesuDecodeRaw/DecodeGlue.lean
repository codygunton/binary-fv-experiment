...
the exported decoder's `ImplementsFunctionInstance` contract hands back at the wrapper's exit state.

Stating it at the *concrete* `canonicalEnvironment`/`Elflings.canonicalDecoderGlobalsLayout` instead
...
theorem freshGlobals_error (error : Contracts.DecodeError) :
...
names. -/
@[simp] theorem freshGlobals_error_pointer (error : Contracts.DecodeError) (resultBuffer : Nat) :
...
The contract's meaning is `meaningDecode input`; the root theorem's goal names `BinaryFv.Specs.SSZ.decode
...
theorem returnCode_of_spec_rejects (grounds : catalogGroundsInSpec) {input : ByteArray}
    (inputBound : input.size < 2 * 1024 * 1024)
    (rejects : BinaryFv.Specs.SSZ.decode input = .rejected) {before after : State}
    (h : CanonicalDecodeExit input before after) :
...
theorem statusCategory_of_freshRejection {error : Contracts.DecodeError}
