...
private theorem writeSuccessFrameLower_of_mainFrameLower
    ...
    (insideWriteFrame : stackPointer - 0x880 ≤ address) :
    stackPointer - 0xbb0 ≤ address := by
  calc
    stackPointer - 0xbb0 ≤ stackPointer - 0x880 :=
      Nat.sub_le_sub_left (by decide : 0x880 ≤ 0xbb0) stackPointer
  ...
  | success decoded =>
      ...
      have writeEntry : WriteSuccessEntry writeArgs callState := by
        refine ⟨(by show 0x14d10 ∈ Elflings.writeSuccessExitPcs; native_decide),
          (Nat.le_trans (by decide : 0x880 ≤ 0xbb0) entry.stackLower),
        ...
        · refine
          ...
          · intro address lower upper
            change args.stackPointer - 0x880 ≤ address at lower
