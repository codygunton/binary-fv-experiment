...
import BinaryFv.Zesu.Validation.GeneratedSourceFunctionVectors

/-!
# Checking every sourceFunction's Lean meaning

The generated vectors describe typed calls to all 43 sourceFunction identities and their exact expected
value or local error. This module evaluates the handwritten Lean `meaning` for each vector and checks
the result with `native_decide`. The host Zig probe independently calls the real source sourceFunction on the
same vector. Agreement with the shared expectation therefore connects the source sourceFunction to its Lean
meaning.
...
open BinaryFv.Zesu.Validation.GeneratedSourceFunctionVectors
...
def bvHex {n : Nat} (v : BinaryFv.Specs.SSZ.RawByteVector n) : String := bytesToHex v.toArray.toList

/-- A variable byte-list field as hex. -/
def rbHex (a : BinaryFv.Specs.SSZ.RawBytes) : String := bytesToHex a.toList
...
/-- The scalar-read meaning selected by sourceFunction name, as an `Option Nat` (`none` = `invalidSsz`). -/
def scalarMeaning (sourceFunction : String) (bytes : ByteArray) (offset : Nat) : Option Nat :=
  if sourceFunction == "ssz_raw.readU32" then (meaningReadU32 bytes offset).toOption.map UInt32.toNat
  else if sourceFunction == "ssz_raw.readU64" then (meaningReadU64 bytes offset).toOption.map UInt64.toNat
  else if sourceFunction == "ssz_raw.readU256" then (meaningReadU256 bytes offset).toOption.map BitVec.toNat
  else (meaningReadOffset bytes offset).toOption

/-- The slice-read meaning selected by sourceFunction name (`bytesAt` vs `readArray[N]`, `len` = the width). -/
def sliceMeaning (sourceFunction : String) (bytes : ByteArray) (offset len : Nat) : Option ByteArray :=
  if sourceFunction == "ssz_raw.bytesAt" then (meaningBytesAt bytes offset len).toOption
...
0/1 presence bit) — the SAME order the Zig probe and `ssz_source_function_vectors.py` use, so all three encode
...
def errLabelOf : DecodeError → String
...
def flatForkActivation (fa : BinaryFv.Specs.SSZ.RawForkActivation) : List Nat :=
  flatOptU64 fa.blockNumber ++ flatOptU64 fa.timestamp

def flatBlob : Option BinaryFv.Specs.SSZ.RawBlobSchedule → List Nat
  | some s => [1, s.target.toNat, s.max.toNat, s.baseFeeUpdateFraction.toNat]
  | none => [0, 0, 0, 0]

def flatForkConfig (fc : BinaryFv.Specs.SSZ.RawForkConfig) : List Nat :=
  fc.fork.toNat :: (flatForkActivation fc.activation ++ flatBlob fc.blobSchedule)

def flatChainConfig (cc : BinaryFv.Specs.SSZ.RawChainConfig) : List Nat :=
...
def containerEnc {α} (flat : α → List Nat) (r : Except DecodeError α) : Option (List Nat) × String :=
  match r with
  | .ok v => (some (flat v), "")
...
def collEnc {α} (render : α → List String) (r : Except DecodeError (Array α)) :
...
def treeBv {n : Nat} (v : BinaryFv.Specs.SSZ.RawByteVector n) : VTree := .leaf (bvHex v)
def treeRb (a : BinaryFv.Specs.SSZ.RawBytes) : VTree := .leaf (rbHex a)
def treeU256 (v : BitVec 256) : VTree := .leaf (bitvec256ToHexLE v)

def renderWithdrawal (w : BinaryFv.Specs.SSZ.RawWithdrawal) : VTree :=
  .node [treeU64 w.index, treeU64 w.validatorIndex, treeBv w.address, treeU64 w.amount]
def renderDeposit (d : BinaryFv.Specs.SSZ.RawDepositRequest) : VTree :=
  .node [treeBv d.pubkey, treeBv d.withdrawalCredentials, treeU64 d.amount, treeBv d.signature, treeU64 d.index]
def renderWithdrawalReq (wr : BinaryFv.Specs.SSZ.RawWithdrawalRequest) : VTree :=
  .node [treeBv wr.sourceAddress, treeBv wr.validatorPubkey, treeU64 wr.amount]
def renderConsolidation (c : BinaryFv.Specs.SSZ.RawConsolidationRequest) : VTree :=
  .node [treeBv c.sourceAddress, treeBv c.sourcePubkey, treeBv c.targetPubkey]

def renderExecutionRequests (er : BinaryFv.Specs.SSZ.RawExecutionRequests) : VTree :=
...
def renderExecutionWitness (ew : BinaryFv.Specs.SSZ.RawExecutionWitness) : VTree :=
  .node [.node (ew.state.toList.map treeRb), .node (ew.codes.toList.map treeRb),
    .node (ew.headers.toList.map treeRb)]

def renderExecutionPayload (ep : BinaryFv.Specs.SSZ.RawExecutionPayload) : VTree :=
...
def renderNewPayloadRequest (npr : BinaryFv.Specs.SSZ.RawNewPayloadRequest) : VTree :=
...
def containerTreeEnc {α} (render : α → VTree) (r : Except DecodeError α) : Option VTree × String :=
  match r with
  | .ok v => (some (render v), "")
  | .error e => (none, errLabelOf e)
...
def treeOptBlob (v : Option BinaryFv.Specs.SSZ.RawBlobSchedule) : VTree :=
  match v with
  | some s => .node [treeU64 s.target, treeU64 s.max, treeU64 s.baseFeeUpdateFraction]
  | none => .node []

def renderForkActivation (fa : BinaryFv.Specs.SSZ.RawForkActivation) : VTree :=
  .node [treeOptU64 fa.blockNumber, treeOptU64 fa.timestamp]
def renderForkConfig (fc : BinaryFv.Specs.SSZ.RawForkConfig) : VTree :=
  .node [treeU64 fc.fork, renderForkActivation fc.activation, treeOptBlob fc.blobSchedule]
def renderChainConfig (cc : BinaryFv.Specs.SSZ.RawChainConfig) : VTree :=
  .node [treeU64 cc.chainId, renderForkConfig cc.activeFork]

def renderStatelessInput (si : BinaryFv.Specs.SSZ.RawV4) : VTree :=
...
def decodeStatusCode : Except DecodeError BinaryFv.Specs.SSZ.RawV4 → Nat
  | .ok _ => 1
  | .error .invalidSsz => 2
  | .error .unknownFork => 3
  | .error .outOfMemory => 4
