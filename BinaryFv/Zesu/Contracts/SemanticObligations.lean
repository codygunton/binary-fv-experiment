...
* **Error taxonomy.** Which errors a function instance can produce at all. These follow from the shape of the
...
`BinaryFv.Zesu.root_compliance` via `binary_is_canonical`'s pinned-artifact facts, so nothing new
...
statements that some function instance cannot reach two of them. Proving those one meaning at a time would
...
def FailsOnly {α : Type} (allowed : DecodeError → Prop)
    (result : Except DecodeError α) : Prop :=
  ∀ error, result = .error error → allowed error

namespace FailsOnly

theorem of_ok {α : Type} {allowed : DecodeError → Prop} (value : α) :
    FailsOnly allowed (.ok value) := by
  intro error h
  exact absurd h (by simp)

theorem of_error {α : Type} {allowed : DecodeError → Prop} {error : DecodeError}
...
theorem bind {α β : Type} {allowed : DecodeError → Prop} {x : Except DecodeError α}
    {f : α → Except DecodeError β} (hx : FailsOnly allowed x)
  ...
  | error e =>
      ...
      have heq : e = error := by
        have hβ : (Except.error e : Except DecodeError β) = Except.error error := h
        simpa using hβ
      exact hx error (congrArg Except.error heq)
  | ok value => exact hf value error h

theorem map {α β : Type} {allowed : DecodeError → Prop} {f : α → β}
    {x : Except DecodeError α} (hx : FailsOnly allowed x) : FailsOnly allowed (x.map f) := by
  intro error h
  cases x with
  | error e =>
      have heq : e = error := by
...
theorem ite {α : Type} {allowed : DecodeError → Prop} {c : Prop} [Decidable c]
    {a b : Except DecodeError α} (ha : FailsOnly allowed a) (hb : FailsOnly allowed b) :
    FailsOnly allowed (if c then a else b) := by
  split <;> assumption

/-- Weakening: a computation that fails only with `p` fails only with any `q` above `p`. -/
theorem mono {α : Type} {p q : DecodeError → Prop} (h : ∀ error, p error → q error)
    {result : Except DecodeError α} (hr : FailsOnly p result) : FailsOnly q result :=
...
theorem ne {α : Type} {allowed : DecodeError → Prop} {result : Except DecodeError α}
    (hr : FailsOnly allowed result) {error : DecodeError} (h : ¬ allowed error) :
    result ≠ .error error :=
  fun heq => h (hr error heq)

end FailsOnly

/-- The predicate almost every function instance satisfies: `invalidSsz` is its only failure. -/
abbrev OnlyInvalid {α : Type} (result : Except DecodeError α) : Prop :=
  FailsOnly (· = .invalidSsz) result

/-- The predicate `decodeForkConfig` and its one caller satisfy. -/
abbrev OnlyInvalidOrFork {α : Type} (result : Except DecodeError α) : Prop :=
  FailsOnly (fun error => error = .invalidSsz ∨ error = .unknownFork) result

theorem OnlyInvalid.toFork {α : Type} {result : Except DecodeError α} (h : OnlyInvalid result) :
...
attribute [local irreducible] BinaryFv.Specs.SSZ.decodeCanonical
...
This is also the existence half of `onlyForkConfigRaisesUnknownFork` — "only" needs a function instance that
actually raises it, not just function instances that do not. -/
...
theorem decodeCanonical_eq (schema : SSZType) (body : ByteArray) :
    BinaryFv.Specs.SSZ.decodeCanonical schema body =
  ...
  unfold BinaryFv.Specs.SSZ.decodeCanonical
...
theorem meaningEmptyIsNone_holds : meaningEmptyIsNone := by
  ...
  simp [meaningEmptyIsNone, meaningOptionalBlobSchedule, decodeCanonical_eq,
    optionalBlobScheduleType, SSZType.deserialize, SSZType.isFixedSize, BinaryFv.Specs.SSZ.blobScheduleType,
    BinaryFv.Specs.SSZ.u64, SSZType.fixedByteSize, SSZType.deserializeFixedElems, SSZType.serialize,
    SSZType.allFixedSize, SSZType.fixedByteSizeFields, BinaryFv.Specs.SSZ.maxBlobSchedulesPerFork,
    hserialize, hempty]
...
theorem meaningOtherLengthIsInvalid_holds : meaningOtherLengthIsInvalid := by
  ...
  · simp [meaningOptionalBlobSchedule, decodeCanonical_eq, optionalBlobScheduleType,
      SSZType.deserialize, SSZType.isFixedSize, BinaryFv.Specs.SSZ.blobScheduleType, BinaryFv.Specs.SSZ.u64,
      SSZType.fixedByteSize, SSZType.allFixedSize, SSZType.fixedByteSizeFields,
      BinaryFv.Specs.SSZ.maxBlobSchedulesPerFork, hbig, sszToDecodeError]
  · have htrailing : bytes.size / 24 * 24 ≠ bytes.size := by omega
    simp [meaningOptionalBlobSchedule, decodeCanonical_eq, optionalBlobScheduleType,
      SSZType.deserialize, SSZType.isFixedSize, BinaryFv.Specs.SSZ.blobScheduleType, BinaryFv.Specs.SSZ.u64,
      SSZType.fixedByteSize, SSZType.allFixedSize, SSZType.fixedByteSizeFields,
      BinaryFv.Specs.SSZ.maxBlobSchedulesPerFork, hbig, htrailing, sszToDecodeError]
...
theorem emptyByteListListIsEmptyArray_holds : emptyByteListListIsEmptyArray := by
  ...
  have hserialize := SSZType.serializeVarElemsAux.eq_1 (BinaryFv.Specs.SSZ.u8.list maxItemBytes) 0
  have hempty : (ByteArray.empty == ByteArray.empty) = true := by decide
  simp [meaningByteListList, decodeCanonical_eq, byteListListType, BinaryFv.Specs.SSZ.byteList,
...
theorem readUInt32LE_zero_of_readU32LE (bytes : ByteArray) (size : bytes.size ≥ 4)
    (zero : BinaryFv.Specs.SSZ.readU32LE? bytes 0 = some 0) : readUInt32LE bytes 0 = some 0 := by
  rw [BinaryFv.Specs.SSZ.readU32LE?] at zero
  split at zero
...
`BinaryFv.Specs.SSZ.readU32LE? (bytes.extract 2 bytes.size) 0 = some 16`, while the entry meaning reads that
...
`BinaryFv.Specs.SSZ.readU32LE?` be applied to a decode path that reads with `readUInt32LE`. -/
theorem readU32LE?_eq_map_readUInt32LE (bytes : ByteArray) (offset : Nat) :
    BinaryFv.Specs.SSZ.readU32LE? bytes offset = (readUInt32LE bytes offset).map UInt32.toNat := by
  rw [BinaryFv.Specs.SSZ.readU32LE?, readUInt32LE]
  split
  · rw [dif_neg (by omega)]
    rfl
  · rw [dif_pos (by omega)]
...
theorem readU32LE?_of_meaningReadOffset (bytes : ByteArray) (offset value : Nat)
    ...
    BinaryFv.Specs.SSZ.readU32LE? bytes offset = some value := by
...
theorem blobScheduleType_deserialize (bytes : ByteArray) (first second third : UInt64)
    ...
    BinaryFv.Specs.SSZ.blobScheduleType.deserialize bytes = .ok ((first, second, third, ()), 24) := by
  simp only [BinaryFv.Specs.SSZ.blobScheduleType, BinaryFv.Specs.SSZ.u64, SSZType.deserialize,
...
theorem blobScheduleType_serialize (first second third : UInt64) :
    BinaryFv.Specs.SSZ.blobScheduleType.serialize (first, second, third, ()) =
      uint64LE first ++ uint64LE second ++ uint64LE third := by
  simp only [BinaryFv.Specs.SSZ.blobScheduleType, BinaryFv.Specs.SSZ.u64, SSZType.serialize,
...
theorem meaningTwentyFourIsSome_holds : meaningTwentyFourIsSome := by
  ...
  have byteSize : BinaryFv.Specs.SSZ.blobScheduleType.fixedByteSize = 24 := by
    simp [BinaryFv.Specs.SSZ.blobScheduleType, BinaryFv.Specs.SSZ.u64, SSZType.fixedByteSize,
      SSZType.fixedByteSizeFields]
  have oneElement :
      BinaryFv.Specs.SSZ.blobScheduleType.serializeFixedElems [(first, second, third, ())] = bytes := by
    simp [SSZType.serializeFixedElems, fixedSize, encoded]
  refine ⟨BinaryFv.Specs.SSZ.rawBlobScheduleOf (first, second, third, ()), ?_⟩
  unfold meaningOptionalBlobSchedule
...
theorem catalogGroundsInSpec_of_agreement (agrees : sourceShapedDecodeAgreesWithOracle) :
  ...
  unfold BinaryFv.Specs.SSZ.decode
  cases BinaryFv.Specs.SSZ.decodeStatelessInput bytes with
  | ok value => simp
  | error error => simp
...
theorem unscopedGrounds_contradicts_ereGate :
  ...
  | ok value =>
      ...
      rw [BinaryFv.Specs.SSZ.decode, tooLarge] at h
      exact absurd h (by simp)
...
theorem acceptanceAgreement_of_valueAgreement
  ...
  | error error =>
      cases ho : BinaryFv.Specs.SSZ.decodeStatelessInput bytes with
      | error e => rfl
      | ok value =>
...
theorem meaningDecodeRaw_of_not_schemaId (bytes : ByteArray) (fits : bytes.size ≤ 4294967295)
    (noSchema : BinaryFv.Specs.SSZ.hasSchemaId bytes = false) :
...
theorem readU32LE_ereGateWitnessOf (n : Nat) (h : 4 ≤ n) :
    BinaryFv.Specs.SSZ.readU32LE? (ereGateWitnessOf n) 0 = some 4294967292 := by
  have inBounds : ¬ (0 + 4 > (ereGateWitnessOf n).size) := by
    rw [size_ereGateWitnessOf]; omega
  simp [BinaryFv.Specs.SSZ.readU32LE?, inBounds, get_ereGateWitnessOf_zero n (by omega),
...
theorem hasSchemaId_ereGateWitnessOf_tail (n : Nat) (h : 4 < n) :
    BinaryFv.Specs.SSZ.hasSchemaId ((ereGateWitnessOf n).extract 4 (ereGateWitnessOf n).size) = false := by
  simp [BinaryFv.Specs.SSZ.hasSchemaId, get_ereGateWitnessOf_tail_zero n h]
...
theorem not_ereRetryAboveGateAt_empty : ¬ ereRetryAboveGateAt ByteArray.empty := by
  rintro ⟨-, prefixExact, -, -, -⟩
  exact absurd prefixExact (by simp [meaningHasExactErePrefix, BinaryFv.Specs.SSZ.readU32LE?])
