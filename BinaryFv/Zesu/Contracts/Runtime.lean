...
/-!
# Runtime and accessor function instances

The allocator vtable, `memcpy`, `memmove`, and the two exported result accessors.

These are the only function instances in the catalog whose identity comes from the symbol table rather than
...
def meaningAlloc (heap : BinaryFv.Zesu.Runtime.BumpHeap) (bytes alignment : Nat) :
    Option (Nat × BinaryFv.Zesu.Runtime.BumpHeap) :=
  BinaryFv.Zesu.Runtime.allocate heap bytes alignment
...
contract, and gives callers a fact they did not have. Non-allocating function instances need no analogue —
...
def postAlloc (env : DecoderEnvironment) (args : AllocArgs)
  ...
  | .ok address =>
      ...
      ∃ cursorBefore cursorAfter,
        ...
        -- interval, and the compiled `zesu_raw_alloc` writes a frame like any other function instance. The
...
def contractAlloc (env : DecoderEnvironment) (heap : BinaryFv.Zesu.Runtime.BumpHeap) :
    FunctionContract DecodeError AllocArgs Nat where
  meaning := fun args =>
...
def contractMemcpy (env : DecoderEnvironment) :
    FunctionContract DecodeError CopyArgs ByteArray where
...
def correctnessClaimAlloc (env : DecoderEnvironment) (heap : BinaryFv.Zesu.Runtime.BumpHeap)
...
def satisfiableAlloc (env : DecoderEnvironment) (heap : BinaryFv.Zesu.Runtime.BumpHeap) : Prop :=
...
other cataloged function instance rather than bare predicates. Both read the private decoder globals rather
...
the same reason they are on the wrapper, and the frame clause is true of this function instance for a sharper
...
/-!
## Allocator vtable function instances
...
can account for every reachable runtime function instance rather than leaving the allocator closure implicit.
-/
...
def contractAllocatorResize (env : DecoderEnvironment) :
    FunctionContract DecodeError Unit Bool where
...
def contractAllocatorRemap (env : DecoderEnvironment) :
    FunctionContract DecodeError Unit Nat where
  meaning := fun _ => .ok 0
  pre := fun _ state => env.CodeIntact state
  post := fun _ result before after =>
    env.CodeIntact after ∧ env.NoAllocation before after ∧
...
clause was added to remove.** A total frame is satisfiable only by a function instance that never touches the
...
function instance can satisfy, and it still says the allocator state and every caller-visible byte survive
untouched, which is the whole content of "free is a no-op" here. -/
def contractAllocatorFree (env : DecoderEnvironment) :
    FunctionContract DecodeError Unit Unit where
  meaning := fun _ => .ok ()
  pre := fun _ state => env.CodeIntact state
  post := fun _ result before after =>
    env.CodeIntact after ∧ env.NoAllocation before after ∧
...
def contractAllocatorAlloc (env : DecoderEnvironment) (heap : BinaryFv.Zesu.Runtime.BumpHeap) :
    FunctionContract DecodeError AllocArgs Nat :=
...
def contractAllocatorCtor (env : DecoderEnvironment) :
    FunctionContract DecodeError AllocatorCtorArgs Unit where
  meaning := fun _ => .ok ()
  pre := fun _ state => env.CodeIntact state
...
def satisfiableAllocatorAlloc (env : DecoderEnvironment)
    (heap : BinaryFv.Zesu.Runtime.BumpHeap) : Prop :=
...
those two function instances; the obligation that the compiled vtable entries really return those constants is
