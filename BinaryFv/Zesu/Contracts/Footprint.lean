import BinaryFv.Zesu.Contracts.RepresentationAudit
...
theorem forkActivation_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawForkActivation) :
...
* **The size is DERIVED, never a literal.** `range base 32` with a hand-written 32 is unfalsifiable —
  ...
  from `BinaryFv.Zesu.Artifacts.fork_activation_layout`, the same compiler-reflected manifest the ABI and allocator
...
`BinaryFv.Zesu.Artifacts.fork_activation_layout` rather than written down, so a layout change surfaces here instead
of silently widening the footprint. -/
theorem forkActivation_footprint_abi (base : Nat) (value : BinaryFv.Specs.SSZ.RawForkActivation)
    {recordSize : Nat} (hsize : BinaryFv.Zesu.Artifacts.forkActivationSize = some recordSize) :
    MemDeterminedOn (range base recordSize) (fun s => ForkActivationRep s base value) := by
  have h32 : recordSize = 32 := by
    have hlayout := BinaryFv.Zesu.Artifacts.fork_activation_layout.1
    rw [hsize] at hlayout
...
theorem localTo_canonicalRepForkActivation_record {recordSize : Nat}
    (hsize : BinaryFv.Zesu.Artifacts.forkActivationSize = some recordSize) :
...
theorem forkActivation_range32_not_tight (base : Nat) :
    ∃ (value : BinaryFv.Specs.SSZ.RawForkActivation) (s1 s2 : State),
...
theorem optionU64_footprint_abi (base : Nat) (value : Option UInt64)
    {recordSize : Nat} (hsize : BinaryFv.Zesu.Artifacts.optionalU64Size = some recordSize) :
    MemDeterminedOn (range base recordSize) (fun s => OptionU64Rep s base value) := by
  have h16 : recordSize = 16 := by
    have hlayout := BinaryFv.Zesu.Artifacts.optional_u64_layout.1
    rw [hsize] at hlayout
    exact Option.some.inj hlayout
  subst h16
...
manifest in `Artifacts.fork_chain_config_layout`. So the nesting introduces **no padding of its own** —
...
theorem blobSchedule_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawBlobSchedule) :
...
theorem forkConfig_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawForkConfig) :
...
theorem chainConfig_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawChainConfig) :
...
theorem localTo_canonicalRepForkConfig_record {recordSize : Nat}
    (hsize : BinaryFv.Zesu.Artifacts.forkConfigSize = some recordSize) :
    LocalTo canonicalRepForkConfig (fun base => range base recordSize) := by
  have h72 : recordSize = 72 := by
    have hlayout := BinaryFv.Zesu.Artifacts.fork_chain_config_layout.1
    rw [hsize] at hlayout
    exact Option.some.inj hlayout
  subst h72
...
theorem localTo_canonicalRepChainConfig_record {recordSize : Nat}
    (hsize : BinaryFv.Zesu.Artifacts.chainConfigSize = some recordSize) :
    LocalTo canonicalRepChainConfig (fun base => range base recordSize) := by
  have h80 : recordSize = 80 := by
    have hlayout := BinaryFv.Zesu.Artifacts.fork_chain_config_layout.2.2.2.2.1
    rw [hsize] at hlayout
    exact Option.some.inj hlayout
  subst h80
...
theorem heapFixedVectorArray_footprint {length : Nat} (base : Nat)
    (values : Array (BinaryFv.Specs.SSZ.RawByteVector length)) :
...
theorem rawWithdrawal_footprint_readSet (base : Nat) (value : BinaryFv.Specs.SSZ.RawWithdrawal) :
...
theorem rawWithdrawal_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawWithdrawal) :
...
theorem rawWithdrawalRequest_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawWithdrawalRequest) :
...
theorem rawDepositRequest_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawDepositRequest) :
...
theorem heapWithdrawalArray_footprint (base : Nat) (values : Array BinaryFv.Specs.SSZ.RawWithdrawal) :
...
theorem heapWithdrawalRequestArray_footprint (base : Nat)
    (values : Array BinaryFv.Specs.SSZ.RawWithdrawalRequest) :
...
theorem heapConsolidationRequestArray_footprint (base : Nat)
    (values : Array BinaryFv.Specs.SSZ.RawConsolidationRequest) :
...
theorem heapDepositRequestArray_footprint (base : Nat)
    (values : Array BinaryFv.Specs.SSZ.RawDepositRequest) :
...
theorem rawWithdrawal_footprint_abi (base : Nat) (value : BinaryFv.Specs.SSZ.RawWithdrawal)
    {recordSize : Nat} (hsize : BinaryFv.Zesu.Artifacts.rawWithdrawalSize = some recordSize) :
    MemDeterminedOn (range base recordSize) (fun s => RawWithdrawalRep s base value) := by
  have h48 : recordSize = 48 := by
    have hlayout := BinaryFv.Zesu.Artifacts.heap_element_size_layout.1
    rw [hsize] at hlayout
    exact Option.some.inj hlayout
  subst h48
  exact rawWithdrawal_footprint base value

theorem heapWithdrawalArray_footprint_abi (base : Nat) (values : Array BinaryFv.Specs.SSZ.RawWithdrawal)
    {elementSize : Nat} (hsize : BinaryFv.Zesu.Artifacts.rawWithdrawalSize = some elementSize) :
...
theorem heapWithdrawalRequestArray_footprint_abi (base : Nat)
    (values : Array BinaryFv.Specs.SSZ.RawWithdrawalRequest) {elementSize : Nat}
    (hsize : BinaryFv.Zesu.Artifacts.rawWithdrawalRequestSize = some elementSize) :
    MemDeterminedOn (range base (elementSize * values.size))
      (fun s => HeapWithdrawalRequestArrayRep s base values) := by
  have h80 : elementSize = 80 := by
    have hlayout := BinaryFv.Zesu.Artifacts.heap_element_size_layout.2.2.1
    rw [hsize] at hlayout
    exact Option.some.inj hlayout
  subst h80
...
theorem heapConsolidationRequestArray_footprint_abi (base : Nat)
    (values : Array BinaryFv.Specs.SSZ.RawConsolidationRequest) {elementSize : Nat}
    (hsize : BinaryFv.Zesu.Artifacts.rawConsolidationRequestSize = some elementSize) :
    MemDeterminedOn (range base (elementSize * values.size))
      (fun s => HeapConsolidationRequestArrayRep s base values) := by
  have h116 : elementSize = 116 := by
    have hlayout := BinaryFv.Zesu.Artifacts.heap_element_size_layout.2.2.2
    rw [hsize] at hlayout
    exact Option.some.inj hlayout
  subst h116
...
theorem heapDepositRequestArray_footprint_abi (base : Nat)
    (values : Array BinaryFv.Specs.SSZ.RawDepositRequest) {elementSize : Nat}
    (hsize : BinaryFv.Zesu.Artifacts.rawDepositRequestSize = some elementSize) :
    MemDeterminedOn (range base (elementSize * values.size))
      (fun s => HeapDepositRequestArrayRep s base values) := by
  have h192 : elementSize = 192 := by
    have hlayout := BinaryFv.Zesu.Artifacts.heap_element_size_layout.2.1
    rw [hsize] at hlayout
    exact Option.some.inj hlayout
  subst h192
...
module's call. -/
theorem heapLayer_footprints_abi (base : Nat) (withdrawal : BinaryFv.Specs.SSZ.RawWithdrawal)
    (withdrawals : Array BinaryFv.Specs.SSZ.RawWithdrawal)
    (withdrawalRequests : Array BinaryFv.Specs.SSZ.RawWithdrawalRequest)
    (consolidations : Array BinaryFv.Specs.SSZ.RawConsolidationRequest)
    (deposits : Array BinaryFv.Specs.SSZ.RawDepositRequest) {sizeW sizeR sizeC sizeD : Nat}
    (hw : BinaryFv.Zesu.Artifacts.rawWithdrawalSize = some sizeW)
    (hr : BinaryFv.Zesu.Artifacts.rawWithdrawalRequestSize = some sizeR)
    (hc : BinaryFv.Zesu.Artifacts.rawConsolidationRequestSize = some sizeC)
    (hd : BinaryFv.Zesu.Artifacts.rawDepositRequestSize = some sizeD) :
...
def zeroWithdrawal : BinaryFv.Specs.SSZ.RawWithdrawal where
  index := 0
  validatorIndex := 0
...
theorem rawWithdrawal_range48_not_tight (base : Nat) :
    ∃ (value : BinaryFv.Specs.SSZ.RawWithdrawal) (s1 s2 : State),
...
def zeroWithdrawalRequest : BinaryFv.Specs.SSZ.RawWithdrawalRequest where
...
theorem rawWithdrawalRequest_range80_not_tight (base : Nat) :
    ∃ (value : BinaryFv.Specs.SSZ.RawWithdrawalRequest) (s1 s2 : State),
...
/-- Every field zero. -/
def zeroDepositRequest : BinaryFv.Specs.SSZ.RawDepositRequest where
...
/-- Every field zero. -/
def zeroConsolidationRequest : BinaryFv.Specs.SSZ.RawConsolidationRequest where
  sourceAddress := Vector.replicate 20 0
...
theorem heapWithdrawalArray_with_presence_tight (base offset : Nat) (hoffset : offset < 48)
    (hwrap : base + 48 ≤ 2 ^ 64) :
    ∃ (values : Array BinaryFv.Specs.SSZ.RawWithdrawal) (s1 s2 : State),
...
theorem executionRequests_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawExecutionRequests)
...
theorem executionRequests_footprint_abi (base : Nat) (value : BinaryFv.Specs.SSZ.RawExecutionRequests)
    (s1 : State) (established : ExecutionRequestsRep s1 base value)
    {recordSize depositSize withdrawalSize consolidationSize : Nat}
    (hrecord : BinaryFv.Zesu.Artifacts.executionRequestsSize = some recordSize)
    (hdeposit : BinaryFv.Zesu.Artifacts.rawDepositRequestSize = some depositSize)
    (hwithdrawal : BinaryFv.Zesu.Artifacts.rawWithdrawalRequestSize = some withdrawalSize)
    (hconsolidation :
      BinaryFv.Zesu.Artifacts.rawConsolidationRequestSize = some consolidationSize) :
    ∃ depositsBase withdrawalsBase consolidationsBase,
      ∀ s2 : State,
        ...
        ExecutionRequestsRep s2 base value := by
  have h48 : recordSize = 48 := by
    have hlayout := BinaryFv.Zesu.Artifacts.allocating_container_size_layout.1
...
theorem executionRequests_survives_sibling (base : Nat) (value : BinaryFv.Specs.SSZ.RawExecutionRequests)
...
theorem executionWitness_footprint (inputBase : Nat) (input : ByteArray) (base : Nat)
    (value : BinaryFv.Specs.SSZ.RawExecutionWitness) (s1 : State)
...
theorem executionWitness_survives_sibling (inputBase : Nat) (input : ByteArray) (base : Nat)
    (value : BinaryFv.Specs.SSZ.RawExecutionWitness) (s1 s2 : State) (ownedSibling : Region)
    (established : ExecutionWitnessRep s1 inputBase input base value)
    (writes : WritesOnlyWithin ownedSibling s1 s2) :
    ∃ stateBase codesBase headersBase,
      (∀ address, executionWitnessRegion base 48 stateBase codesBase headersBase
...
theorem executionWitness_footprint_abi (inputBase : Nat) (input : ByteArray) (base : Nat)
    ...
    (established : ExecutionWitnessRep s1 inputBase input base value)
    {recordSize : Nat}
    (hrecord : BinaryFv.Zesu.Artifacts.executionWitnessSize = some recordSize) :
    ∃ stateBase codesBase headersBase,
      ∀ s2 : State,
        ...
        ExecutionWitnessRep s2 inputBase input base value := by
  have h48 : recordSize = 48 := by
    have hlayout := BinaryFv.Zesu.Artifacts.allocating_container_size_layout.2.1
    rw [hrecord] at hlayout
    exact Option.some.inj hlayout
  subst h48
...
theorem executionPayloadFixed_footprint (base : Nat) (value : BinaryFv.Specs.SSZ.RawExecutionPayload) :
...
theorem executionPayload_footprint (inputBase : Nat) (input : ByteArray) (base : Nat)
    (value : BinaryFv.Specs.SSZ.RawExecutionPayload) (s1 : State)
...
def zeroExecutionPayload : BinaryFv.Specs.SSZ.RawExecutionPayload where
...
theorem executionPayload_range592_not_tight (base : Nat) :
    ∃ (value : BinaryFv.Specs.SSZ.RawExecutionPayload) (s1 s2 : State),
...
theorem executionPayload_footprint_abi (inputBase : Nat) (input : ByteArray) (base : Nat)
    ...
    (hrecord : BinaryFv.Zesu.Artifacts.executionPayloadSize = some recordSize)
    (hwithdrawal : BinaryFv.Zesu.Artifacts.rawWithdrawalSize = some withdrawalSize) :
    ∃ transactionsBase withdrawalsBase,
      ∀ s2 : State,
  ...
  have h592 : recordSize = 592 := by
    have hlayout := BinaryFv.Zesu.Artifacts.allocating_container_size_layout.2.2.1
    rw [hrecord] at hlayout
    exact Option.some.inj hlayout
...
def newPayloadRequestRegion (base recordSize transactionsBase payloadWithdrawalsBase
    ...
    (value : BinaryFv.Specs.SSZ.RawNewPayloadRequest) : Region :=
  Region.union (range base recordSize)
    (Region.union
...
theorem newPayloadRequest_footprint (inputBase : Nat) (input : ByteArray) (base : Nat)
    (value : BinaryFv.Specs.SSZ.RawNewPayloadRequest) (s1 : State)
...
theorem newPayloadRequest_footprint_abi (inputBase : Nat) (input : ByteArray) (base : Nat)
    ...
    (established : NewPayloadRequestRep s1 inputBase input base value)
    {recordSize : Nat}
    (hrecord : BinaryFv.Zesu.Artifacts.newPayloadRequestSize = some recordSize) :
    ∃ transactionsBase payloadWithdrawalsBase versionedHashesBase depositsBase
      requestWithdrawalsBase consolidationsBase,
      ∀ s2 : State,
  ...
  have h688 : recordSize = 688 := by
    have hlayout := BinaryFv.Zesu.Artifacts.allocating_container_size_layout.2.2.2
    rw [hrecord] at hlayout
    exact Option.some.inj hlayout
  subst h688
...
theorem rawStatelessInput_footprint (base recordSize : Nat)
    (hsize : BinaryFv.Zesu.Artifacts.rawStatelessInputSize = some recordSize) :
...
/-- The ten heap regions in the public footprint order. This proof inventory does not define the
public heap-footprint contract. -/
def statelessInputHeapRegions (value : BinaryFv.Specs.SSZ.StatelessInput)
    (bases : StatelessInputDescriptorBases) : List Region :=
  [range bases.versionedHashesBase (value.newPayloadRequest.versionedHashes.size * 32),
    ...
    range bases.consolidationRequestsBase
      (value.newPayloadRequest.executionRequests.consolidations.size * 116),
    range bases.witnessStateBase (value.witness.state.size * 16),
    range bases.witnessCodesBase (value.witness.codes.size * 16),
    range bases.witnessHeadersBase (value.witness.headers.size * 16),
    range bases.publicKeysBase (value.publicKeys.size * 65)]

theorem mem_statelessInputHeapRegion {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} {region : Region} {address : Nat}
    (member : region ∈ statelessInputHeapRegions value bases) (inside : region address) :
    statelessInputHeapRegion value bases address := by
  simp [statelessInputHeapRegions] at member
  rcases member with rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl | rfl
  · exact Or.inl inside
  · exact Or.inr (Or.inl inside)
  · exact Or.inr (Or.inr (Or.inl inside))
  · exact Or.inr (Or.inr (Or.inr (Or.inl inside)))
  · exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inl inside))))
  · exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl inside)))))
  · exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl inside))))))
  · exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl inside)))))))
  · exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inl inside))))))))
  · exact Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr (Or.inr inside))))))))

/-- The fixed-vector representation states its byte window as `stride * count`; this explicit
membership theorem connects that producer order to the public `count * stride` footprint. -/
theorem versionedHashContentsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.versionedHashesBase (32 * value.newPayloadRequest.versionedHashes.size) ∈
      statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
  left
  congr 1
  omega

theorem transactionDescriptorsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.transactionsBase (16 * value.newPayloadRequest.executionPayload.transactions.size) ∈
      statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
  right; left
  congr 1
...
theorem withdrawalContentsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.withdrawalsBase (48 * value.newPayloadRequest.executionPayload.withdrawals.size) ∈
      statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
  right; right; left
  congr 1
...
theorem depositContentsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.depositsBase (192 * value.newPayloadRequest.executionRequests.deposits.size) ∈
      statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
...
theorem withdrawalRequestContentsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.withdrawalRequestsBase (80 * value.newPayloadRequest.executionRequests.withdrawals.size) ∈
      statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
...
theorem consolidationRequestContentsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.consolidationRequestsBase
        (116 * value.newPayloadRequest.executionRequests.consolidations.size) ∈
      statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
...
theorem witnessStateDescriptorsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.witnessStateBase (16 * value.witness.state.size) ∈ statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
  right; right; right; right; right; right; left
  congr 1
  omega

theorem witnessCodesDescriptorsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.witnessCodesBase (16 * value.witness.codes.size) ∈ statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
  right; right; right; right; right; right; right; left
  congr 1
  omega

theorem witnessHeadersDescriptorsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.witnessHeadersBase (16 * value.witness.headers.size) ∈
      statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
...
theorem publicKeyContentsHeapRegion_member {value : BinaryFv.Specs.SSZ.StatelessInput}
    {bases : StatelessInputDescriptorBases} :
    range bases.publicKeysBase (65 * value.publicKeys.size) ∈ statelessInputHeapRegions value bases := by
  simp only [statelessInputHeapRegions, List.mem_cons]
...
/-- Agreement on the complete root-and-heap footprint gives agreement on any selected heap array. -/
theorem heapRegionAgreement {before after : State} {rootBase rootSize : Nat}
    ...
    {region : Region} {address : Nat}
    (inside : region address)
    (member : region ∈ statelessInputHeapRegions value bases := by simp [statelessInputHeapRegions]) :
    before.mem.get? address = after.mem.get? address :=
  agree address (Or.inr (mem_statelessInputHeapRegion member inside))
...
theorem statelessInputInputSlices_footprint (inputBase : Nat) (input : ByteArray) (rootBase rootSize : Nat)
    ...
    StatelessInputInputSlicesRep s2 inputBase input rootBase value bases d2 := by
  exact
    { extraData := h.extraData.imp fun _ h' => h'.imp fun _ h'' =>
      ...
      transactions := inputSliceDescriptorArray_footprint inputBase input bases.transactionsBase _
        s1 s2 (fun address inside => heapRegionAgreement agree inside transactionDescriptorsHeapRegion_member) h.transactions
      witnessState := inputSliceDescriptorArray_footprint inputBase input bases.witnessStateBase _
        s1 s2 (fun address inside => heapRegionAgreement agree inside witnessStateDescriptorsHeapRegion_member) h.witnessState
      witnessCodes := inputSliceDescriptorArray_footprint inputBase input bases.witnessCodesBase _
        s1 s2 (fun address inside => heapRegionAgreement agree inside witnessCodesDescriptorsHeapRegion_member) h.witnessCodes
      witnessHeaders := inputSliceDescriptorArray_footprint inputBase input bases.witnessHeadersBase _
        s1 s2 (fun address inside => heapRegionAgreement agree inside witnessHeadersDescriptorsHeapRegion_member) h.witnessHeaders }
  ...
  exact
    { root := rawStatelessInput_footprint rootBase rootSize hsize s1 s2 agreeRecord allocation.root
      ...
      versionedHashContents := heapFixedVectorArray_footprint bases.versionedHashesBase _ s1 s2
        (fun address inside => heapRegionAgreement agree inside versionedHashContentsHeapRegion_member)
      ...
      withdrawalContents := heapWithdrawalArray_footprint bases.withdrawalsBase _ s1 s2
        (fun address inside => heapRegionAgreement agree inside withdrawalContentsHeapRegion_member)
      ...
      depositContents := heapDepositRequestArray_footprint bases.depositsBase _ s1 s2
        (fun address inside => heapRegionAgreement agree inside depositContentsHeapRegion_member)
      ...
      withdrawalRequestContents :=
        heapWithdrawalRequestArray_footprint bases.withdrawalRequestsBase _ s1 s2
          (fun address inside => heapRegionAgreement agree inside withdrawalRequestContentsHeapRegion_member)
      ...
      consolidationRequestContents :=
        heapConsolidationRequestArray_footprint bases.consolidationRequestsBase _ s1 s2
          (fun address inside => heapRegionAgreement agree inside consolidationRequestContentsHeapRegion_member)
      ...
      publicKeyContents := publicKeyContents s1 s2
        (fun address inside => heapRegionAgreement agree inside publicKeyContentsHeapRegion_member) allocation.publicKeyContents }
...
theorem containerLayer_footprints_abi (inputBase : Nat) (input : ByteArray)
    ...
    (hWitnessSize : BinaryFv.Zesu.Artifacts.executionWitnessSize = some witnessSize)
    (hPayloadSize : BinaryFv.Zesu.Artifacts.executionPayloadSize = some payloadSize)
    (hNewPayloadSize : BinaryFv.Zesu.Artifacts.newPayloadRequestSize = some newPayloadSize)
    (hRootSize : BinaryFv.Zesu.Artifacts.rawStatelessInputSize = some rootSize)
    (hDeposit : BinaryFv.Zesu.Artifacts.rawDepositRequestSize = some depositSize)
    (hRequestWithdrawal :
      BinaryFv.Zesu.Artifacts.rawWithdrawalRequestSize = some requestWithdrawalSize)
    (hConsolidation :
      BinaryFv.Zesu.Artifacts.rawConsolidationRequestSize = some consolidationSize)
    (hWithdrawal : BinaryFv.Zesu.Artifacts.rawWithdrawalSize = some withdrawalSize) :
