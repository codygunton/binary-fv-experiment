...
/-- One byte read outside the written region costs one membership check. The memory-side counterpart
of `BinaryFv.RiscV.WritesOnlyRegs.get`, and stated the same way so the two automate alike. -/
theorem WritesOnlyWithin.get {owned : Region} {before after : State}
    (h : WritesOnlyWithin owned before after) (address : Nat) (outside : ¬ owned address) :
    after.mem.get? address = before.mem.get? address := h address outside

/-! ### Automating memory preservation

`grind_pattern` for the same reason as on the register side: the conclusion
`after.mem.get? a = before.mem.get? a` does not mention `owned`, and the antecedent
`WritesOnlyWithin owned before after` does not mention `a`, so neither single-sided attribute can
determine the instantiation and both are rejected. The multi-pattern over both does.

The membership equations below are what let it discharge the side condition. Unlike the register
side -- where the sets are `@[reducible]` and `decide` closes membership -- a `Region` is a predicate
over `Nat`, so `¬ range base size a` is an *arithmetic* obligation. Registering the defining
equivalences lets `grind` see through the constructors and hand the residue to its linear-arithmetic
core, rather than making the constructors `@[reducible]`, which would change elaboration everywhere
they appear.

Verified to close: a read outside a single `range`; outside a `Region.union` of a `range` and an
`interval` (the shape `allocatedRegion` takes); through a two-step chain at *different* regions; and
the realistic symbolic case, where the base is a variable and separation is a hypothesis rather than
a literal. Verified to refuse: an address inside the written range, and one inside the second region
of a chain. -/

@[grind =] theorem range_apply (base size address : Nat) :
    range base size address ↔ (base ≤ address ∧ address < base + size) := Iff.rfl

@[grind =] theorem interval_apply (before after address : Nat) :
    interval before after address ↔ (before ≤ address ∧ address < after) := Iff.rfl

@[grind =] theorem Region.union_apply (r1 r2 : Region) (address : Nat) :
    Region.union r1 r2 address ↔ (r1 address ∨ r2 address) := Iff.rfl

grind_pattern WritesOnlyWithin.get => WritesOnlyWithin owned before after, after.mem.get? address

section GrindRegression
variable {s t u : State}

/-- Outside a single range. -/
example (h : WritesOnlyWithin (range 0x1000 16) s t) :
    t.mem.get? 0x2000 = s.mem.get? 0x2000 := by grind

/-- Outside a union, and through a chain at two different regions, with no intermediate state. -/
example (h1 : WritesOnlyWithin (Region.union (range 0x1000 16) (interval 0x3000 0x3100)) s t)
    (h2 : WritesOnlyWithin (interval 0x4000 0x4100) t u) :
    u.mem.get? 0x2000 = s.mem.get? 0x2000 := by grind

/-- The realistic case: a symbolic base, separated by a hypothesis rather than by literals. -/
example (base : Nat) (sep : base + 16 ≤ 0x2000) (h : WritesOnlyWithin (range base 16) s t) :
    t.mem.get? 0x2000 = s.mem.get? 0x2000 := by grind

/-- Must refuse an address inside the written region; otherwise the three above would be equally
satisfied by a rule that dropped its side condition. -/
example (_h : WritesOnlyWithin (range 0x1000 16) s t) : True := by
  fail_if_success (have : t.mem.get? 0x1004 = s.mem.get? 0x1004 := by grind)
  trivial

/-- And inside the second region of a chain. -/
example (_h1 : WritesOnlyWithin (range 0x1000 16) s t)
    (_h2 : WritesOnlyWithin (interval 0x3000 0x3100) t u) : True := by
  fail_if_success (have : u.mem.get? 0x3050 = s.mem.get? 0x3050 := by grind)
  trivial
