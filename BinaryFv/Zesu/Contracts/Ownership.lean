import BinaryFv.Zesu.Contracts.FrameGap
...
"This function instance writes nowhere else" is false of real function instances: they use scratch stack and they
allocate. The true fact is "this function instance writes only within what it **owns**". And "siblings do not
...
matched it.* `DecoderEnvironment.ownedRegion` now names all four things a function instance owns — record,
...
call site, rather than a clause every function instance re-proves.
...
namespace BinaryFv.Zesu.Contracts.Ownership

open BinaryFv.RiscV
open BinaryFv.Zesu.Contracts.FrameGap
...
that did not would be false of every compiled function instance; so a record that lived *in* the stack really
...
theorem fixed_container_cannot_clobber_sibling {α : Type} {env : DecoderEnvironment}
    ...
    {result : Except DecodeError α} {value : BitVec 8} {s1 s2 : State}
...
end BinaryFv.Zesu.Contracts.Ownership
