...
/-- **Relocation leaves the `SourceFunctionSpec` untouched.** The exported wrapper's shared specification is
...
theorem accessors_after_fresh_rejection (env : DecoderEnvironment) (globals : DecoderGlobalsLayout)
    (resultBuffer : Nat) (error : DecodeError) :
    (contractRawResult env globals resultBuffer).meaning
...
The runs are shaped like the function instances they stand for rather than doing nothing. Each writes its stack
frame — a function instance that touched no memory would satisfy the memory clauses for a reason having nothing
...
left it alone would be a witness for a function instance that executed nothing — and, worse, it would satisfy a
`platformPreserved` that wrongly included the counter, hiding the fact that such a clause is false of
every real function instance. With the advance in place, adding `minstret` to `platformPreserved` breaks
...
strongest thing that is still shaped like a compiled function instance — it touches exactly one byte, in its
...
function instance.** It wrote its stack frame, returned the status in `a0`, retired instructions, and came back
...
theorem raw_error_run_satisfies_the_clauses (model : DecoderGlobalsModel) :
    postRawError calleeFrameEnv model (.ok model.status.code) accessorBefore
      ...
      -- it really wrote its frame: not a function instance that does nothing
...
def postRawResultHistorical (env : DecoderEnvironment) (resultBuffer : Nat)
    (model : DecoderGlobalsModel) (result : Except DecodeError Nat)
...
theorem postRawError_eq_historical_and_clauses (env : DecoderEnvironment)
    (model : DecoderGlobalsModel) (result : Except DecodeError Nat) (before after : State) :
