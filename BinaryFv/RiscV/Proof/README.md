...
`ImageLoadFrame.lean` proves that the sparse image, input, and fill loaders write the intended bytes
and leave every other address unchanged. `RunnerCorrespondence.lean` connects a proved sentinel trace
to the executable runner's result.
