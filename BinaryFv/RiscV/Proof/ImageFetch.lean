...
/-!
# Lifting an image-owned word to the generated fetch

Ownership of an encoded word by a `ProgramImage`, and the lift of that ownership to the generated
fetch inputs (`FetchBytesAt`).

This is the *generic* half of artifact fetching: which words a particular pinned binary owns is a
target fact, but turning "the image owns this word" into "the machine fetches these four bytes" is
target-independent.
-/
