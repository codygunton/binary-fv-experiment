...
```
environment already contains 'Register.enumToBitVec' from BinaryFv.Keccak.MemcpyContract
```

which is exactly what blocked `BinaryFv.Keccak.MemcpyContract` and `BinaryFv.Keccak.CallStepContract`
from co-elaborating (see `BinaryFv.Keccak.CumulativeImports`).
...
namespace BinaryFv.RiscV.SailEnumAux
...
end BinaryFv.RiscV.SailEnumAux

open Lean Elab Command Meta in
run_cmd liftTermElabM do
  for e in BinaryFv.RiscV.SailEnumAux.enums do
