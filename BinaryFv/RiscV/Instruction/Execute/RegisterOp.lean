...
`Runs`-level execution contracts for the generated register-writing ALU instructions used by
typical helper loops and outlined functions:

* `execute_ITYPE` — `ADDI`, `XORI`, `ANDI`, `ORI` (immediate ALU ops),
* `execute_RTYPE` — `ADD`, `AND`,
