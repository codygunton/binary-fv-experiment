...
/-- A generated source read followed by a non-stack write preserves `x2` on all outcomes. -/
...
/-- Every generated shift-immediate writer preserves `x2` off the stack destination. -/
theorem execute_SHIFTIOP_preserves_stack_pointer (state : State) (shamt : BitVec 6)
    (source destination : regidx) (op : sop) (notStack : destination ≠ stackPointer) :
    (match (execute_SHIFTIOP shamt source destination op).run state with
    | .ok _ state' => state'.regs.get? x2
    | .error _ state' => state'.regs.get? x2) = state.regs.get? x2 := by
  cases op with
  | SLLI =>
    simpa [execute_SHIFTIOP] using
  ...
  | SRAI =>
    simpa [execute_SHIFTIOP] using
      write_after_rX_preserves_stack_pointer state source destination
        (fun value =>
          LeanRV64DExecutable.Functions.shift_bits_right_arith value
            (Sail.BitVec.extractLsb shamt (LeanRV64DExecutable.Functions.log2_xlen -i 1) 0))
        notStack
