...
theorem fileBytesMatchMemory_afterByteWrites (image : ProgramImage)
    ...
    (code : image.fileBytesMatchMemory state.mem) :
    image.fileBytesMatchMemory (afterByteWrites state writes).mem := by
  induction writes generalizing state with
  ...
  | cons write writes ih =>
      ...
      have afterHead := image.fileBytesMatchMemory_insert_non_file
...
theorem fileBytesMatchMemory_afterWriteBytes (image : ProgramImage)
    ...
    (code : image.fileBytesMatchMemory state.mem) :
    image.fileBytesMatchMemory (afterWriteBytes state address value).mem := by
  apply fileBytesMatchMemory_afterByteWrites image state _
  · intro write member
    obtain ⟨index, rfl⟩ := List.mem_ofFn.mp member
