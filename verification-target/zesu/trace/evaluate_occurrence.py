#!/usr/bin/env python3
"""Map production-ELF evidence to a generated Elfling occurrence and evaluate its function-instance binding
...
load addresses; `SourceFunctionSpec.meaning` (the decoded RawBlobSchedule from the actual loads); result +
...
CHILD_OFFSETS = {117: 0, 118: 8, 119: 16}  # Const-offset bindings for the three readU64 children
...
def evaluate_compact(ev, mutate=None):
    """Evaluate the compact evidence against the fixed function-instance binding, meaning, and layout. `mutate` is an
    ...
    # slice_ptr + {0,8,16} (the function-instance const-offset bindings) — no gap, no extra, no shift. This ties the
...
def capture_arms(qemu, gdb, plugin, elf, program_json, arms, scratch):
    ...
    occ = json.load(open(program_json))["function_instances"][116]
