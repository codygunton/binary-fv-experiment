#!/usr/bin/env python3
"""Scale per-occurrence production-ELF validation from the blob-schedule vertical slice to
...
executes, never inherited from another occurrence of the same source source_function. Every check that cannot
...
The generic per-occurrence checks (apply to all occurrences, trace-only):
  ...
  withinStepBound       max per-invocation instruction count <= the source_function's contract step bound
                        (const bounds + readArray's specialization width; input-dependent bounds = gap)
  allocationConsistent  a NON-allocating source_function never bumps the allocator cursor (.sbss); the
                        allocation ACT is a cursor store. Heap stores alone are allowed — a
                        non-allocating source_function may fill a buffer a caller allocated (and memcpy/memmove
  ...
  meaningTie            source_function-specific: for scalar/offset/slice leaf readers the value read from the
...
def reduce_occurrence(occ, short, catalog, executed, loads, stores, input_len=0,
                      children_pcs=frozenset(), dynamic_pcs=frozenset(),
                      external_call_entries=frozenset()):
    ...
    f: dict[str, object] = {
        "index": occ.get("index"),
        "qualified": occ["qualified"], "source_function": short,
    ...
    for i in range(len(executed) - 1):
        ...
        if s in owned:
            if s in dynamic_pcs:
                ...
                exec_owned.add((s, t))
            # A direct call temporarily leaves the caller's regions but is not a caller exit. The
            # generated `externalCalls` inventory identifies those targets from the pinned ELF.
            if not in_region(t) and t not in external_call_entries:
                leaving_src.add(s)
    ...
    # meaning-tie witness: a value the source_function both LOADED from input and STORED (scalar carried through).
    ...
    # (<= the source_function's slice length); copyLen = in-region store count (<= bytes copied); inputLen = the
...
def occ_to_lean(rec) -> str:
    ...
    ev = (
        ...
        f"sourceFunction := {_str(f['source_function'])}, arm := {_str(rec.get('arm') or '')}, "
...
def emit_report(records, summary) -> str:
    ...
    L = ["# Scaled per-occurrence production-ELF coverage (GENERATED)",
         "",
         "Regenerated by `verification-target/zesu/trace/scale_occurrences.py` from the UNCHANGED production",
         "`zesu-ssz` ELF under pinned `qemu-riscv64`. Diagnostic-only; never imported by the proof.",
         "Coverage is PER OCCURRENCE (never inherited from a sibling of the same source_function). `P`=pass,",
         ...
         "(`UNCOVERED_CLASSIFICATION.md` / `classify_uncovered.py`), not merely untested. The",
         "evidence conclusions exclude them and the one unresolved step bound.",
         "",
    ...
    if uncovered:
        ...
        for r in uncovered:
            reason = DOCUMENTED_UNCOVERED.get(r["source_function"], "not exercised by any tested arm")
            L.append(f"- occ {r['index']} `{r['qualified']}` — {reason}")
    L += ["",
          "Header abbreviations: " + ", ".join(f"{short[n]}={n}" for n in CHECK_NAMES) + ".",
          "",
          "| idx | source_function | arm | " + " | ".join(short[n] for n in CHECK_NAMES) + " | gaps |",
    ...
    for r in records:
        ...
        L.append(f"| {r['index']} | {r['source_function']} | {r.get('arm') or '-'} | {cells} | {gap} |")
    return "\n".join(L) + "\n"


def emit_lean(records) -> str:
    head = [
        "-- GENERATED — do not edit. Regenerated by verification-target/zesu/trace/scale_occurrences.py from the",
        ...
        "",
        "namespace ZesuVerification.GeneratedScaleEvidence",
    ...
    tail = ["", "end ZesuVerification.GeneratedScaleEvidence", ""]
    return "\n".join(head) + "\n" + body + "\n" + "\n".join(tail)
...
def main() -> int:
    ...
    ap.add_argument("--catalog", default=str(HERE / "source_function_catalog.json"))
    ...
    occ = program["function_instances"]
    excluded_function_instances = program.get("excludedFunctionInstances", [])
    ...
    for idx, o in enumerate(occ):
        ...
        for kind, target in o.get("externalCalls", []):
            if kind == "function_instance":
                external_entries.add(occ[target]["entryPc"])
            elif kind == "excl":
                external_entries.add(excluded_function_instances[target]["entryPc"])
        o = {**o, "index": idx}
        ...
        for spec in args.arm:
            ...
            if any(any(a <= pc < b for a, b in ranges) for pc in executed):
                ...
                break
        if chosen is None:
            facts = reduce_occurrence(
                o, short, catalog, [], [], [], 0, kids, dyn_pcs, external_entries
            )  # covered=False
        else:
            (executed, loads, stores), _, ipath = arm_traces[chosen]
            input_len = Path(ipath).stat().st_size
            facts = reduce_occurrence(
                o, short, catalog, executed, loads, stores, input_len, kids, dyn_pcs,
                external_entries
            )
        checks, gaps = evaluate_facts(facts)
        records.append({
            "index": idx, "qualified": o["qualified"], "source_function": short,
