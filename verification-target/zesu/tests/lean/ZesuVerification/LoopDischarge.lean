...
/-!
# Loop composition diagnostics

Test-only. Nothing here is imported by the root theorem, and this module does not prove the behavior
or termination of a production Zesu loop. It establishes how the trace types can represent an
input-dependent loop and checks whether the generated function-instance boundaries permit the real
machine loops to be reached at all.
...
3. **The trace types can express a loop of input-dependent trip count, with no new
...
4. **A function instance that is its own callee is inadmissible**
   (`callGraphRanked_forbids_self_edge`): a
