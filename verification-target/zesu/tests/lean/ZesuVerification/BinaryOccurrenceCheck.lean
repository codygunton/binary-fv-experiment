...
/-!
# Production-binary diagnostic checker for the decodeOptionalBlobSchedule occurrence

Evaluates the compact production-ELF evidence (`GeneratedBinaryEvidence`) for occurrence 116 and its
three nested `readU64` children (117/118/119) against their fixed function-instance bindings, handwritten meaning,
...
/-- The const-offset bindings of the three nested `readU64` function instances. -/
...
/-- Whether the input loads are EXACTLY the three 8-byte windows at the bound offsets 0/8/16 — no gap,
...
the diagnostic). These are the same eight classes as the Python `negative_tests.py`, ported across the boundary.
-/
