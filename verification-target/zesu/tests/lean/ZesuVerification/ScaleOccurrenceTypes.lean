/-!
# Types for scaled per-occurrence production-ELF validation

Compact, deterministic per-occurrence evidence (`OccScaleEvidence`) and the eight generic check results
(`ScaleChecks`) for the scaled production-evidence validator. One `OccScaleEvidence` per occurrence in `program.json`
...
structure OccScaleEvidence where
  ...
  arm : String
  /-- Declared function-instance entry PC. -/
  entryPc : Nat
