...
Schema (`ssz-contract-corpus-v1`), one JSON object per line:
  ...
  source_function     : the pinned `FunctionId` string the case exercises (e.g. `ssz_raw.decode`)
  args        : typed arguments (for the top-level source_function, `{input: <hex>}`)
  ...
  expect      : minimal cross-check the corpus can assert without a reference decoder
                (`{accept: bool}` for the top-level source_function); full value/ledger are runner output
"""
...
def top_level_rows(fixtures) -> list[dict]:
    ...
    for case in fixtures.cases():
        rows.append({
            ...
            "source_function": "ssz_raw.decode",
...
def emit_lean_corpus(fixtures, max_bytes: int = 65536) -> str:
    ...
    lines = [
        "-- GENERATED by verification-target/zesu/tests/ssz_contract_corpus.py --out-lean. DO NOT EDIT.",
        ...
        "namespace BinaryFv.Zesu.Validation.GeneratedCorpus",
    ...
    lines.append(",\n".join(rows))
    lines.append("]")
    lines.append("end BinaryFv.Zesu.Validation.GeneratedCorpus")
    return "\n".join(lines) + "\n"
